...
macro("${VCPKG_OVERRIDE_FIND_PACKAGE_NAME}" z_vcpkg_find_package_package_name)
    ...
    # Fixed in CMake 3.15
    cmake_path(IS_PREFIX CMAKE_FIND_ROOT_PATH "${VCPKG_CMAKE_FIND_ROOT_PATH}" IS_PREFIXED)
    if(CMAKE_SYSTEM_NAME STREQUAL "iOS" AND NOT IS_PREFIXED)
    ...
    if(EXISTS "${z_vcpkg_find_package_vcpkg_cmake_wrapper_path}")
        if(DEFINED ARGS)
            ...
            unset(z_vcpkg_find_package_${z_vcpkg_find_package_package_name}_backup_ARGS)
        endif()
        set(ARGS "${z_vcpkg_find_package_package_name};${ARGN}")
        include("${z_vcpkg_find_package_vcpkg_cmake_wrapper_path}")
        if(DEFINED z_vcpkg_find_package_${z_vcpkg_find_package_package_name}_backup_ARGS)
            set(ARGS "${z_vcpkg_find_package_${z_vcpkg_find_package_package_name}_backup_ARGS}")
        else()
            unset(ARGS)
        endif()
    elseif(z_vcpkg_find_package_package_name STREQUAL "Boost" AND EXISTS "${_VCPKG_INSTALLED_DIR}/${VCPKG_TARGET_TRIPLET}/include/boost")
        ...
        _find_package("${z_vcpkg_find_package_package_name}" ${ARGN})
    elseif(z_vcpkg_find_package_package_name STREQUAL "ICU" AND EXISTS "${_VCPKG_INSTALLED_DIR}/${VCPKG_TARGET_TRIPLET}/include/unicode/utf.h")
        set(z_vcpkg_find_package_ICU_ARGN "${ARGN}")
        list(FIND z_vcpkg_find_package_ICU_ARGN "COMPONENTS" z_vcpkg_find_package_COMPONENTS_IDX)
        if(NOT z_vcpkg_find_package_COMPONENTS_IDX EQUAL "-1")
            _find_package("${z_vcpkg_find_package_package_name}" ${ARGN} COMPONENTS data)
        else()
            _find_package("${z_vcpkg_find_package_package_name}" ${ARGN})
        endif()
