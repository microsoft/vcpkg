...
#[===[.md:
# z_vcpkg_find_configurations(<prefix> [INCLUDE_NAMES <names...>] [LIBRARY_NAMES <names...>] [LIBRARY_AS_FILEPATH])

Finds the include path, release binary and debug binary of library,
and sets common variables. This function is for vcpkg cmake wrappers,
to be called before _find_package(...).

When given INCLUDE_NAMES, it sets the <prefix>_INCLUDE_DIR cache variable to the
path of the first directory containing one of the listed names.

When given LIBRARY_NAMES, it sets the following variables:
<prefix>_LIBRARY_RELEASE (cache variable): release library.
<prefix>_LIBRARY_DEBUG (cache variable): debug library.
<prefix>_LIBRARY: When the LIBRARY_AS_FILEPATH keyword is given, a cache variable
with the variant matching the current build type. Otherwise a normal variable
which evaluates to the right library variant at cmake generation time.

To properly support the VCPKG_PREFER_SYSTEM_LIBS setting, the names shall include
all common variants suported by the the CMake find module in addition to the
variants which are generated by the vcpkg port.

When the include path or the binaries are requested but cannot be found, an error
is raised.

Synopsis:
    z_vcpkg_find_library_configurations(GDAL INCLUDE_NAMES gdal.h LIBRARY_NAMES gdal gdal_d)
#]===]
function(z_vcpkg_find_configurations prefix)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "LIBRARY_AS_FILEPATH"
        ""
        "INCLUDE_NAMES;LIBRARY_NAMES"
    )
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION} was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    if(arg_INCLUDE_NAMES)
        find_path(${prefix}_INCLUDE_DIR NAMES ${arg_INCLUDE_NAMES})
        if(NOT ${prefix}_INCLUDE_DIR)
            message(FATAL_ERROR "Couldn't find an include directory containing ${arg_INCLUDE_NAMES}. ${${prefix}_INCLUDE_DIR}")
        endif()
    endif()
    if(arg_LIBRARY_NAMES)
        z_vcpkg_find_library_release("${prefix}_LIBRARY_RELEASE" NAMES ${arg_LIBRARY_NAMES} NAMES_PER_DIR)
        if(NOT ${prefix}_LIBRARY_RELEASE)
            message(FATAL_ERROR "Couldn't find a library matching ${arg_LIBRARY_NAMES}.")
        endif()
        z_vcpkg_find_library_debug("${prefix}_LIBRARY_DEBUG" NAMES ${arg_LIBRARY_NAMES} NAMES_PER_DIR)
        if(NOT arg_LIBRARY_AS_FILEPATH)
            include(SelectLibraryConfigurations)
            select_library_configurations("${prefix}")
            set(${prefix}_LIBRARY "${${prefix}_LIBRARY}" PARENT_SCOPE)
        elseif(Z_VCPKG_BUILD_TYPE STREQUAL "debug")
            set(${prefix}_LIBRARY "${${prefix}_LIBRARY_DEBUG}" CACHE FILEPATH "")
        else()
            set(${prefix}_LIBRARY "${${prefix}_LIBRARY_RELEASE}" CACHE FILEPATH "")
        endif()
    endif()
endfunction()
...
#[===[.md:
# z_vcpkg_fixup_configurations(<prefix> [TARGET <target>])

Updates <prefix>_LIBRARIES and targets to provide release and debug binary of
a library. This function is for vcpkg cmake wrappers, to be called after
_find_package(...).

This function checks if the <prefix>_LIBRARIES variable starts with a single
config variant, and replaces that config with an expression that properly combines
<prefix>_LIBRARY_DEBUG and <prefix>_LIBRARY_DEBUG.

When given a TARGET, it sets IMPORTED_LOCATION_DEBUG and IMPORTED_LOCATION_RELEASE
if these properties are missing from the target.

Synopsis:
    z_vcpkg_fixup_configurations(GDAL TARGET GDAL::GDAL)
#]===]
function(z_vcpkg_fixup_configurations prefix)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        ""
        "TARGET"
        ""
    ...
    if(NOT "${${prefix}_LIBRARY_DEBUG}" OR NOT "${${prefix}_LIBRARY_RELEASE}")
        message(FATAL_ERROR "${prefix}_LIBRARY_DEBUG or ${prefix}_LIBRARY_RELEASE is not set.")
    endif()
    if(DEFINED "${prefix}_LIBRARIES")
        string(FIND "${${prefix}_LIBRARIES}" "${${prefix}_LIBRARY}" index)
        if(index EQUAL 0)
            list(REMOVE_ITEM "${prefix}_LIBRARIES" "${${prefix}_LIBRARY}")
            list(INSERT "${prefix}_LIBRARIES" 0 optimized "${${prefix}_LIBRARY_RELEASE}" debug "${${prefix}_LIBRARY_DEBUG}")
        endif()
    endif()
    if(arg_TARGET)
        get_property(configurations TARGET "${arg_TARGET}" PROPERTY IMPORTED_CONFIGURATIONS)
        if(NOT "RELEASE" IN_LIST configurations)
            set_property(TARGET "${arg_TARGET}" APPEND PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
            set_property(TARGET "${arg_TARGET}" APPEND PROPERTY IMPORTED_LOCATION_RELEASE "${${prefix}_LIBRARY_RELEASE}")
        endif()
        if(NOT "DEBUG" IN_LIST configurations)
            set_property(TARGET "${arg_TARGET}" APPEND PROPERTY IMPORTED_CONFIGURATIONS DEBUG)
            set_property(TARGET "${arg_TARGET}" APPEND PROPERTY IMPORTED_LOCATION_DEBUG "${${prefix}_LIBRARY_DEBUG}")
        endif()
    endif()
endfunction()
