...
param(
    ...
    [Parameter(Mandatory=$False)][string]$vcpkgToolDir = "",
...
if ([string]::IsNullOrWhiteSpace($vcpkgToolDir)) {
    $vcpkgToolDir=$vcpkgRootDir
}

if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64' -or $env:PROCESSOR_IDENTIFIER -match "ARMv[8,9] \(64-bit\)") {
    & "$scriptsDir/tls12-download-arm64.exe" github.com "/microsoft/vcpkg-tool/releases/download/$versionDate/vcpkg-arm64.exe" "$vcpkgToolDir\vcpkg.exe"
} else {
    & "$scriptsDir/tls12-download.exe" github.com "/microsoft/vcpkg-tool/releases/download/$versionDate/vcpkg.exe" "$vcpkgToolDir\vcpkg.exe"
}
...
& "$vcpkgToolDir\vcpkg.exe" version --disable-metrics

if ($disableMetrics)
{
    Set-Content -Value "" -Path "$vcpkgToolDir\vcpkg.disable-metrics" -Force
}
elseif (-Not (Test-Path "$vcpkgToolDir\vcpkg.disable-metrics"))
{
