...
if ($env:X_VCPKG_TOOL_PATH -eq $null)
{
...
{
    if (Test-Path -Path "$env:X_VCPKG_TOOL_PATH\vcpkg.exe")
    {
        Write-Host "Copying vcpkg.exe from $env:X_VCPKG_TOOL_PATH..."
        Copy-Item "$env:X_VCPKG_TOOL_PATH\vcpkg.exe" -Destination "$vcpkgRootDir"
    }
    else
    {
        Write-Error "X_VCPKG_TOOL_PATH was set but is either an invalid path or does not contain vcpkg.exe."
    }
}
