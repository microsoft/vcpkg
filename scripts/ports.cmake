...
if(CMD MATCHES "^BUILD$")
    ...
    if(VCPKG_TARGET_IS_WINDOWS)
        message(FATAL_ERROR "Breaking WINDOWS builds because I am not interested in those and don't want block windows CI that long!")
    endif()
    ...
    if(CMAKE_HOST_UNIX)
        execute_process(COMMAND find . -xtype l
                WORKING_DIRECTORY ${CURRENT_PACKAGES_DIR}
                OUTPUT_VARIABLE BROKEN_LINKS)
        if(BROKEN_LINKS)
            message(FATAL_ERROR "Port has following broken symlinks:\n${BROKEN_LINKS}")
        endif()
        execute_process(COMMAND find . -type l
                WORKING_DIRECTORY ${CURRENT_PACKAGES_DIR}
                OUTPUT_VARIABLE ALL_LINKS)
        if(ALL_LINKS)
            message(STATUS "Port has following symlinks: ${ALL_LINKS}")
        endif()
        string(REGEX REPLACE "(\\\n|\\\r)" ";" ALL_LINKS "${ALL_LINKS}")
        message(STATUS "Port has following as list symlinks: ${ALL_LINKS}")
        string(REGEX REPLACE "\\\./" "${CURRENT_PACKAGES_DIR}/" ALL_LINKS "${ALL_LINKS}")
        message(STATUS "Port has following absolute symlinks: ${ALL_LINKS}")
        foreach(_link IN LISTS ALL_LINKS)
            if(NOT EXISTS "${_link}")
                continue()
            endif()
            execute_process(COMMAND readlink ${_link}
                WORKING_DIRECTORY ${CURRENT_PACKAGES_DIR}
                OUTPUT_VARIABLE link_path)
            message(STATUS "link_path:${link_path}")
            if(link_path MATCHES "(${CURRENT_PACKAGES_DIR}|${CURRENT_INSTALLED_DIR})")
                message(FATAL_ERROR "Port has absolute symlink:\n${_link_path}")
            endif()
        endforeach()
    endif()
