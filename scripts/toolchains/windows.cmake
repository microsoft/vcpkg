...
if(NOT _CMAKE_IN_TRY_COMPILE)
    ...
    set(CMAKE_CXX_FLAGS_RELEASE "${VCPKG_CRT_LINK_FLAG_PREFIX} /O2 /Oi /Gy /DNDEBUG /Zi /FS ${VCPKG_CXX_FLAGS_RELEASE}" CACHE STRING "")
    set(CMAKE_C_FLAGS_RELEASE "${VCPKG_CRT_LINK_FLAG_PREFIX} /O2 /Oi /Gy /DNDEBUG /Zi /FS ${VCPKG_C_FLAGS_RELEASE}" CACHE STRING "")

    get_property( _CMAKE_IN_TRY_COMPILE GLOBAL PROPERTY IN_TRY_COMPILE )
    if(NOT _CMAKE_IN_TRY_COMPILE)
        string(APPEND CMAKE_SHARED_LINKER_FLAGS " /PDB:${CMAKE_BINARY_DIR}/${VCPKG_PORT}.pdb /PDBALTPATH:%_PDB% /PDBSTRIPPED:${CMAKE_BINARY_DIR}/${VCPKG_PORT}_strip.pdb")
        #string(APPEND CMAKE_STATIC_LINKER_FLAGS " /PDB:${CMAKE_BINARY_DIR}/${VCPKG_PORT}.pdb /PDBALTPATH:%_PDB% /PDBSTRIPPED:${CMAKE_BINARY_DIR}/${VCPKG_PORT}_strip.pdb") lib does not have these options
        string(APPEND CMAKE_EXE_LINKER_FLAGS " /PDB:${CMAKE_BINARY_DIR}/${VCPKG_PORT}.pdb /PDBALTPATH:%_PDB% /PDBSTRIPPED:${CMAKE_BINARY_DIR}/${VCPKG_PORT}_strip.pdb")
        ...
        function(z_vcpkg_install_pdbs)
            ...
            if(EXISTS "${CMAKE_BINARY_DIR}/${VCPKG_PORT}.pdb")
                install("FILES" "${CMAKE_BINARY_DIR}/${VCPKG_PORT}.pdb" "TYPE" "${intall_type}")
            endif()
            if(EXISTS "${CMAKE_BINARY_DIR}/${VCPKG_PORT}_strip.pdb")
                install("FILES" "${CMAKE_BINARY_DIR}/${VCPKG_PORT}_strip.pdb" "TYPE" "${intall_type}")
            endif()
        endfunction()
        cmake_language(DEFER DIRECTORY "${CMAKE_SOURCE_DIR}" CALL z_vcpkg_install_pdbs)
        cmake_language(DEFER DIRECTORY "${CMAKE_SOURCE_DIR}" CALL z_vcpkg_install_pdbs)
    endif()
