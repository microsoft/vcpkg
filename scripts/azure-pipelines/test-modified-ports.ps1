...
if ([string]::IsNullOrWhiteSpace($BinarySourceStub)) {
    ...
    elseif ($BuildReason -eq 'PullRequest') {
        ...
        $prOrCiArgs = @('--allow-unexpected-passing', '--skip-failures')
    }
    else {
...
Copy-Item "scripts/buildsystems/vcpkg.cmake" -Destination "scripts/test_ports/cmake"
Copy-Item "scripts/buildsystems/vcpkg.cmake" -Destination "scripts/test_ports/cmake-user"
& "./vcpkg$executableExtension" ci "--triplet=$Triplet" --x-xunit=$xmlFile --failure-logs=$failureLogs "--ci-baseline=$PSScriptRoot/../ci.baseline.txt" @commonArgs @cachingArgs @parentHashes @prOrCiArgs
