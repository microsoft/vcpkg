...
T objc_call(id obj, const char *selector, Args... args) {
    using FuncPtr = T (*)(id, SEL, Args...);
    return reinterpret_cast<FuncPtr>(objc_msgSend)(obj, sel_registerName(selector), args...);
}
template <typename T, typename... Args>
T objc_call(const char *clazz, const char *selector, Args... args) {
    return objc_call<T>(reinterpret_cast<id>(objc_getClass(clazz)), selector, args...);
}
#endif
