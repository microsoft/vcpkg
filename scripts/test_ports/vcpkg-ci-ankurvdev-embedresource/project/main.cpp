...
void verify_resource(ResourceLoader const& r) {
    if (r.name() == L"main.cpp") {
        ...
        if (r.template data<uint8_t>().size() != MAIN_CPP_FILE_SIZE) {
            throw std::runtime_error("r.data.len() != MAIN_CPP_FILE_SIZE");
        }
        #endif
        #ifdef __cpp_lib_string_view
        if (r.string().size() != MAIN_CPP_FILE_SIZE) {
            throw std::runtime_error("r.string().size() != MAIN_CPP_FILE_SIZE");
        }
        #endif
        ...
        #endif
        #ifdef __cpp_lib_string_view
        if (r.string().size() != CMAKELISTS_TXT_FILE_SIZE) {
            throw std::runtime_error("r.string().size() != CMAKELISTS_TXT_FILE_SIZE");
        }
        #endif
    } else {
