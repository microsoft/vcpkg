...
function(vcpkg_install_meson)
    ...
    if(VCPKG_TARGET_IS_WINDOWS AND VCPKG_LIBRARY_LINKAGE STREQUAL static AND NOT VCPKG_TARGET_IS_MINGW)
        ...
        foreach(gen_library IN LISTS gen_libraries)
            ...
            string(REGEX REPLACE ".a$" ".lib" new_libname "${libname}")
            string(REGEX REPLACE "^lib" "" new_libname "${new_libname}")
            file(RENAME "${gen_library}" "${libdir}/${new_libname}")
            # For cmake fixes.
            string(REGEX REPLACE ".a$" "" old_librawname "${libname}")
            string(REGEX REPLACE ".lib$" "" new_librawname "${new_libname}")
            vcpkg_list(APPEND renamed_libs ${new_librawname})
            set(${librawname}_old ${old_librawname})
            set(${librawname}_new ${new_librawname})
        endforeach()
