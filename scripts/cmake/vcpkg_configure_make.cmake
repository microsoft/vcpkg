...
function(_vcpkg_cmake_to_autotools_arch in_arch out_var)
    unset(${out_var} PARENT_SCOPE)
    if(in_arch MATCHES "(amd|AMD)64" OR in_arch MATCHES "(x|X)86[-_]+64" OR in_arch MATCHES "x64")
        set(${out_var} x86_64 PARENT_SCOPE)
    elseif(in_arch MATCHES "(x|X)86")
        set(${out_var} i686 PARENT_SCOPE)
    elseif(in_arch MATCHES "^(ARM|arm)64$")
        set(${out_var} aarch64 PARENT_SCOPE)
    elseif(in_arch MATCHES "^(ARM|arm)$")
        set(${out_var} arm PARENT_SCOPE)
    endif()
endfunction()

function(_vcpkg_cmake_to_vcpkg_arch in_arch out_var)
    unset(${out_var} PARENT_SCOPE)
    if(in_arch MATCHES "(amd|AMD)64" OR in_arch MATCHES "(x|X)86[-_]+64" OR in_arch MATCHES "x64")
...
function(_vcpkg_cmake_to_autotools_system in_system out_var)
    ...
    elseif(in_system MATCHES "Windows")
        set(${out_var} "pc-mingw32" PARENT_SCOPE)
    elseif(in_system MATCHES "Darwin")
        set(${out_var} "apple-darwin" PARENT_SCOPE)
    elseif(in_system MATCHES "Android")
        set(${out_var} "linux-android" PARENT_SCOPE)
    endif()
endfunction()

macro(_vcpkg_determine_autotools_host_cpu out_var)
    if(DEFINED VCPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR)
        set(_HOST_ARCH ${VCPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR})
    ...
    _vcpkg_cmake_to_autotools_arch(${_HOST_ARCH} ${out_var})
    if (NOT DEFINED ${out_var})
        message(FATAL_ERROR "Unsupported host architecture ${_HOST_ARCH} in _vcpkg_determine_autotools_host_cpu!" )
    endif()
    unset(_HOST_ARCH)
endmacro()

macro(_vcpkg_determine_vcpkg_host_cpu out_var)
    if(DEFINED VCPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR)
        set(_HOST_ARCH ${VCPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR})
    elseif(DEFINED ENV{PROCESSOR_ARCHITEW6432})
        set(_HOST_ARCH $ENV{PROCESSOR_ARCHITEW6432})
    ...
    _vcpkg_cmake_to_vcpkg_arch(${_HOST_ARCH} ${out_var})
    if (NOT DEFINED ${out_var})
        message(FATAL_ERROR "Unsupported host architecture ${_HOST_ARCH} in _vcpkg_determine_autotools_host_cpu!" )
    endif()
    unset(_HOST_ARCH)
...
macro(_vcpkg_determine_autotools_host_os out_var)
    if(DEFINED VCPKG_DETECTED_CMAKE_HOST_SYSTEM_NAME)
        set(_HOST_SYSTEM ${VCPKG_DETECTED_CMAKE_HOST_SYSTEM_NAME})
    elseif(VCPKG_TARGET_IS_WINDOWS)
        set(_HOST_SYSTEM "Windows")
    elseif(VCPKG_TARGET_IS_LINUX)
        set(_HOST_SYSTEM "Linux")
    elseif(VCPKG_TARGET_IS_OSX)
        set(_HOST_SYSTEM "Darwin")
    elseif(VCPKG_TARGET_IS_ANDROID)
        set(_HOST_SYSTEM "Android")
    endif()
    _vcpkg_cmake_to_autotools_system(${_HOST_SYSTEM} ${out_var})
    if (NOT DEFINED ${out_var})
        message(FATAL_ERROR "Unsupported host os ${_HOST_SYSTEM} in _vcpkg_determine_autotools_host_os!" )
    endif()
    unset(_HOST_SYSTEM)
endmacro()

macro(_vcpkg_determine_autotools_target_cpu out_var)
    _vcpkg_cmake_to_autotools_arch(${VCPKG_TARGET_ARCHITECTURE} ${out_var})
    if(NOT DEFINED ${out_var})
...
macro(_vcpkg_determine_autotools_target_os out_var)
    _vcpkg_cmake_to_autotools_system(${VCPKG_DETECTED_CMAKE_SYSTEM_NAME} ${out_var})
    if(NOT DEFINED ${out_var})
...
macro(_vcpkg_select_for_build_detected cmake_var for_build out_var)
    if (${VCPKG_CROSSCOMPILING} AND ${for_build})
        set(${out_var} ${VCPKG_DETECTED_FOR_BUILD_${cmake_var}})
    else()
        set(${out_var} ${VCPKG_DETECTED_${cmake_var}})
    endif()
endmacro()
...
macro(_vcpkg_setup_detected_env env_name cmake_var)
    ...
    set(env_name_for_build ${env_name}_FOR_BUILD)
    _vcpkg_select_for_build_detected(${cmake_var} TRUE ${env_name}_for_build)
    if (${env_name}_for_build)
        set(ENV{${env_name_for_build}} "${${env_name}_for_build}")
    endif()
endmacro()

macro(_vcpkg_extract_cpp_flags_and_set_cflags_and_cxxflags _SUFFIX _FOR_BUILD)
    _vcpkg_select_for_build_detected(CMAKE_C_FLAGS_${_SUFFIX} ${_FOR_BUILD} C_FLAGS)
    _vcpkg_select_for_build_detected(CMAKE_CXX_FLAGS_${_SUFFIX} ${_FOR_BUILD} CXX_FLAGS)
    _vcpkg_select_for_build_detected(CMAKE_OSX_SYSROOT ${_FOR_BUILD} OSX_SYSROOT)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_FLAG_C ${_FOR_BUILD} INCLUDE_FLAG_C)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_FLAG_CXX ${_FOR_BUILD} INCLUDE_FLAG_CXX)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_FLAG_C_SEP ${_FOR_BUILD} INCLUDE_FLAG_C_SEP)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_FLAG_CXX_SEP ${_FOR_BUILD} INCLUDE_FLAG_CXX_SEP)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_SYSTEM_FLAG_C ${_FOR_BUILD} INCLUDE_SYSTEM_FLAG_C)
    _vcpkg_select_for_build_detected(CMAKE_INCLUDE_SYSTEM_FLAG_CXX ${_FOR_BUILD} INCLUDE_SYSTEM_FLAG_CXX)
    if(${_FOR_BUILD})
        set(VAR_SUFFIX ${_SUFFIX}_FOR_BUILD)
    else()
        set(VAR_SUFFIX ${_SUFFIX})
    endif()

    string(REGEX MATCHALL "( |^)-D[^ ]+" CPPFLAGS_${VAR_SUFFIX} "${C_FLAGS}")
    string(REGEX MATCHALL "( |^)-D[^ ]+" CXXPPFLAGS_${VAR_SUFFIX} "${CXX_FLAGS}")
    list(JOIN CXXPPFLAGS_${VAR_SUFFIX} "|" CXXREGEX)
    if(CXXREGEX)
        list(FILTER CPPFLAGS_${VAR_SUFFIX} INCLUDE REGEX "(${CXXREGEX})")
    else()
        set(CPPFLAGS_${VAR_SUFFIX})
    endif()
    list(JOIN CPPFLAGS_${VAR_SUFFIX} "|" CPPREGEX)
    list(JOIN CPPFLAGS_${VAR_SUFFIX} " " CPPFLAGS_${VAR_SUFFIX})
    set(CPPFLAGS_${VAR_SUFFIX} "${CPPFLAGS_${VAR_SUFFIX}}")
    if(CPPREGEX)
        string(REGEX REPLACE "(${CPPREGEX})" "" CFLAGS_${VAR_SUFFIX} "${C_FLAGS}")
        string(REGEX REPLACE "(${CPPREGEX})" "" CXXFLAGS_${VAR_SUFFIX} "${CXX_FLAGS}")
    else()
        set(CFLAGS_${VAR_SUFFIX} "${C_FLAGS}")
        set(CXXFLAGS_${VAR_SUFFIX} "${CXX_FLAGS}")
    endif()
    string(REGEX REPLACE " +" " " CPPFLAGS_${VAR_SUFFIX} "${CPPFLAGS_${VAR_SUFFIX}}")
    string(REGEX REPLACE " +" " " CFLAGS_${VAR_SUFFIX} "${CFLAGS_${VAR_SUFFIX}}")
    string(REGEX REPLACE " +" " " CXXFLAGS_${VAR_SUFFIX} "${CXXFLAGS_${VAR_SUFFIX}}")
    ...
    # just ignore them. 
    string(REGEX REPLACE "((-|/)RTC[^ ]+)" "-Xcompiler \\1" CFLAGS_${VAR_SUFFIX} "${CFLAGS_${VAR_SUFFIX}}")
    string(REGEX REPLACE "((-|/)RTC[^ ]+)" "-Xcompiler \\1" CXXFLAGS_${VAR_SUFFIX} "${CXXFLAGS_${VAR_SUFFIX}}")

    if (OSX_SYSROOT)
        string(APPEND CPPFLAGS_${VAR_SUFFIX} " -isysroot ${OSX_SYSROOT}")
    endif()

    if (${VCPKG_CROSSCOMPILING} AND ${_FOR_BUILD})
        set(VCPKG_INCLUDE_PATH "${CURRENT_HOST_INSTALLED_DIR}/include")
    else()
        set(VCPKG_INCLUDE_PATH "${CURRENT_INSTALLED_DIR}/include")
    endif()
    if ("${INCLUDE_SYSTEM_FLAG_C}" STREQUAL "")
        set(INCLUDE_PREFIX_C ${INCLUDE_FLAG_C}${INCLUDE_FLAG_C_SEP})
    else()
        set(INCLUDE_PREFIX_C ${INCLUDE_SYSTEM_FLAG_C})
    endif()
    if ("${INCLUDE_SYSTEM_FLAG_CXX}" STREQUAL "")
        set(INCLUDE_PREFIX_CXX ${INCLUDE_FLAG_CXX}${INCLUDE_FLAG_CXX_SEP})
    else()
        set(INCLUDE_PREFIX_CXX ${INCLUDE_SYSTEM_FLAG_CXX})
    endif()
    file(TO_NATIVE_PATH "${VCPKG_INCLUDE_PATH}" VCPKG_INCLUDE_PATH)
    set(VCPKG_INCLUDE_PATH "\"${VCPKG_INCLUDE_PATH}\"")

    string(PREPEND CFLAGS_${VAR_SUFFIX} "${INCLUDE_PREFIX_C}${VCPKG_INCLUDE_PATH} ")
    string(PREPEND CXXFLAGS_${VAR_SUFFIX} "${INCLUDE_PREFIX_CXX}${VCPKG_INCLUDE_PATH} ")

    string(STRIP "${CPPFLAGS_${VAR_SUFFIX}}" CPPFLAGS_${VAR_SUFFIX})
    string(STRIP "${CFLAGS_${VAR_SUFFIX}}" CFLAGS_${VAR_SUFFIX})
    string(STRIP "${CXXFLAGS_${VAR_SUFFIX}}" CXXFLAGS_${VAR_SUFFIX})
    debug_message(STATUS "CPPFLAGS_${VAR_SUFFIX}: ${CPPFLAGS_${VAR_SUFFIX}}")
    debug_message(STATUS "CFLAGS_${VAR_SUFFIX}: ${CFLAGS_${VAR_SUFFIX}}")
    debug_message(STATUS "CXXFLAGS_${VAR_SUFFIX}: ${CXXFLAGS_${VAR_SUFFIX}}")
endmacro()
...
function(vcpkg_configure_make)
    ...
    if(VCPKG_CROSSCOMPILING)
        _vcpkg_determine_vcpkg_host_cpu(_host_arch)
        ...
        set(_VCPKG_HOST_CMAKE_VARS_FILE "${_VCPKG_HOST_CMAKE_VARS_FILE}" PARENT_SCOPE)
        debug_message("Including host cmake vars from: ${_VCPKG_HOST_CMAKE_VARS_FILE}")
    ...
    if(CMAKE_HOST_WIN32)
        ...
        if (NOT _csc_BUILD_TRIPLET)
            _vcpkg_determine_autotools_host_cpu(HOST_ARCH)
            _vcpkg_determine_autotools_host_os(HOST_OS)
            _vcpkg_determine_autotools_target_cpu(TARGET_ARCH)
    ...
    if(NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug" AND NOT _csc_NO_DEBUG)
        ...
        if(VCPKG_LIBRARY_LINKAGE STREQUAL "static")
            _vcpkg_select_for_build_detected(
                CMAKE_STATIC_LINKER_FLAGS_${_VAR_SUFFIX} 
                FALSE 
                LINKER_FLAGS_${_VAR_SUFFIX})
            _vcpkg_select_for_build_detected(
                CMAKE_STATIC_LINKER_FLAGS_${_VAR_SUFFIX} 
                TRUE 
                LINKER_FLAGS_${_VAR_SUFFIX}_FOR_BUILD)
        else() # dynamic
            _vcpkg_select_for_build_detected(
                CMAKE_SHARED_LINKER_FLAGS_${_VAR_SUFFIX} 
                FALSE 
                LINKER_FLAGS_${_VAR_SUFFIX})
            _vcpkg_select_for_build_detected(
                CMAKE_SHARED_LINKER_FLAGS_${_VAR_SUFFIX} 
        ...
        _vcpkg_extract_cpp_flags_and_set_cflags_and_cxxflags(${_VAR_SUFFIX} FALSE)
        _vcpkg_extract_cpp_flags_and_set_cflags_and_cxxflags(${_VAR_SUFFIX} TRUE)
        ...
        if (CMAKE_HOST_WIN32 AND VCPKG_DETECTED_FOR_BUILD_CMAKE_C_COMPILER MATCHES "cl.exe")
            if(NOT _vcm_paths_with_spaces)
                set(LDFLAGS_${_VAR_SUFFIX}_FOR_BUILD "-L${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib -L${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib/manual-link")
            endif()
            set(LINK_ENV_${_VAR_SUFFIX}_FOR_BUILD "${LINKER_FLAGS_${_VAR_SUFFIX}_FOR_BUILD}")
        else()
            set(_link_dirs)
            if(EXISTS "${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib")
                set(_link_dirs_for_build "-L${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib")
            endif()
            if(EXISTS "${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib/manual-link")
                set(_link_dirs_for_build "${_link_dirs_for_build} -L${_VCPKG_HOST_INSTALLED}${PATH_SUFFIX_${_VAR_SUFFIX}}/lib/manual-link")
            endif()
            string(STRIP "${_link_dirs_for_build}" _link_dirs_for_build)
            set(LDFLAGS_${_VAR_SUFFIX}_FOR_BUILD "${_link_dirs_for_build} ${LINKER_FLAGS_${_VAR_SUFFIX}_FOR_BUILD}")
        endif()
        unset(_VAR_SUFFIX)
    endif()
    ...
    foreach(_buildtype IN LISTS _buildtypes)
        ...
        endforeach()
        _vcpkg_setup_detected_env(MACOSX_DEPLOYMENT_TARGET CMAKE_OSX_DEPLOYMENT_TARGET)
        ...
        set(ENV{LDFLAGS} "${LDFLAGS_${_buildtype}} ${LINKER_FLAGS}")
        set(ENV{CPPFLAGS_FOR_BUILD} "${CPPFLAGS_${_buildtype}_FOR_BUILD}")
        set(ENV{CFLAGS_FOR_BUILD} "${CFLAGS_${_buildtype}_FOR_BUILD}")
        set(ENV{CXXFLAGS_FOR_BUILD} "${CXXFLAGS_${_buildtype}_FOR_BUILD}")
        set(ENV{LDFLAGS_FOR_BUILD} "${LDFLAGS_${_buildtype}_FOR_BUILD}")
