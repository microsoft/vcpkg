...
macro(_vcpkg_determine_autotools_target_triplet out_var)
    _vcpkg_determine_autotools_target_cpu(_arch)
    _vcpkg_determine_autotools_target_os(_os)
    if (VCPKG_DETECTED_CMAKE_SYSTEM_NAME MATCHES "Android")
        set(_os "${_os}${VCPKG_DETECTED_ANDROID_PLATFORM_LEVEL}")
    endif()
    set(${out_var} "${_arch}-${_os}")
endmacro()
...
function(vcpkg_configure_make)
    ...
    if (CMAKE_HOST_WIN32)
        ...
        if (VCPKG_DETECTED_CMAKE_CROSSCOMPILING)
            ...
            _vcpkg_determine_autotools_target_triplet(TARGET_TRIPLET_ARG)
            if (TARGET_TRIPLET_ARG)
                set(TARGET_TRIPLET_ARG "--target=${TARGET_TRIPLET_ARG}")
            endif()
            set(LDFLAGS_EXTRA)
            foreach (syslib_dir IN LISTS ${VCPKG_DETECTED_CMAKE_SYSTEM_LIBRARY_PATH})
                set(LDFLAGS_EXTRA "${LDFLAGS_EXTRA} -L${syslib_dir}")
            endforeach()
            set(LDFLAGS_EXTRA "${LDFLAGS_EXTRA} --sysroot=${VCPKG_DETECTED_CMAKE_SYSROOT}")
    ...
    foreach(_buildtype IN LISTS _buildtypes)
        ...
        set(ENV{CFLAGS} "${CFLAGS_${_buildtype}} ${TARGET_TRIPLET_ARG}")
        set(ENV{CXXFLAGS} "${CXXFLAGS_${_buildtype}} ${TARGET_TRIPLET_ARG}")
        set(ENV{RCFLAGS} "${VCPKG_DETECTED_CMAKE_RC_FLAGS_${_buildtype}}")
        set(ENV{LDFLAGS} "${LDFLAGS_${_buildtype}} ${LDFLAGS_EXTRA}")
