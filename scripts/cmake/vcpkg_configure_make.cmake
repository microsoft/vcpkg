...
function(vcpkg_configure_make)
    ...
    if(VCPKG_TARGET_IS_ANDROID)
        if (VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID MATCHES "^Clang$")
            ...
            if(VCPKG_HOST_IS_WINDOWS)
                _vcpkg_determine_autotools_host_cpu(BUILD_ARCH) # machine you are building on => --build=
                _vcpkg_determine_host_mingw(BUILD_MINGW)
                list(APPEND _csc_BUILD_TRIPLET "--build=${BUILD_ARCH}-pc-${BUILD_MINGW}")  # This is required since we are running in a msys
                                                                                           # shell which will be otherwise identified as ${BUILD_ARCH}-pc-msys
            elseif(VCPKG_HOST_IS_OSX)
                _vcpkg_determine_autotools_host_arch_mac(BUILD_ARCH) # machine you are building on => --build=
                list(APPEND _csc_BUILD_TRIPLET "--build=${BUILD_ARCH}-apple-darwin${VCPKG_CMAKE_HOST_SYSTEM_VERSION}")
            elseif(VCPKG_HOST_IS_LINUX)
                _vcpkg_determine_autotools_host_cpu(BUILD_ARCH) # machine you are building on => --build=
                list(APPEND _csc_BUILD_TRIPLET "--build=${BUILD_ARCH}-pc-linux-gnu")
            endif()

            if (DEFINED ENV{ANDROID_API_LEVEL})
                set(ANDROID_API_LEVEL $ENV{ANDROID_API_LEVEL} CACHE STRING "")
            else()
                set(ANDROID_API_LEVEL 23 CACHE STRING "")
            endif()
            ...
            if (TARGET_ARCH MATCHES "armv7-a")
                ...
                list(APPEND CONFIGURE_ENV "CC=armv7a-linux-androideabi${ANDROID_API_LEVEL}-clang")
                list(APPEND CONFIGURE_ENV "CXX=armv7a-linux-androideabi${ANDROID_API_LEVEL}-clang++")
            else()
                list(APPEND _csc_BUILD_TRIPLET "--host=${TARGET_ARCH}-linux-android")
                list(APPEND CONFIGURE_ENV "CC=${TARGET_ARCH}-linux-android${ANDROID_API_LEVEL}-clang")
                list(APPEND CONFIGURE_ENV "CXX=${TARGET_ARCH}-linux-android${ANDROID_API_LEVEL}-clang++")
            endif()
            debug_message("Using make triplet: ${_csc_BUILD_TRIPLET}")

            list(APPEND CONFIGURE_ENV "AR=llvm-ar")
            list(APPEND CONFIGURE_ENV "RANLIB=llvm-ranlib")
            list(APPEND CONFIGURE_ENV "READELF=llvm-readelf")
            list(APPEND CONFIGURE_ENV "STRIP=llvm-strip")
        endif()
    endif()
    ...
    elseif(VCPKG_TARGET_IS_ANDROID)
        find_program(base_cmd env REQUIRED)
    else()
    ...
    foreach(_buildtype IN LISTS _buildtypes)
        ...
        elseif(VCPKG_TARGET_IS_WINDOWS)
            set(command "${base_cmd}" -c "${CONFIGURE_ENV} $@" -- "./${RELATIVE_BUILD_PATH}/configure" ${_csc_BUILD_TRIPLET} ${_csc_OPTIONS} ${_csc_OPTIONS_${_buildtype}})
        elseif(VCPKG_TARGET_IS_ANDROID)
            set(command ${base_cmd} ${CONFIGURE_ENV} "./${RELATIVE_BUILD_PATH}/configure" ${_csc_BUILD_TRIPLET} ${_csc_OPTIONS} ${_csc_OPTIONS_${_buildtype}})
        else()
