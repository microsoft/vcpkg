...
macro(z_vcpkg_setup_detected_env env_name cmake_var)
    if(VCPKG_DETECTED_${cmake_var})
        ...
        string(FIND "$ENV{PATH}" "${cmd_PATH}${VCPKG_HOST_PATH_SEPARATOR}" _PathIndex)
        if (NOT _PathIndex EQUAL 0)
            set(ENV{PATH} "${cmd_PATH}${VCPKG_HOST_PATH_SEPARATOR}$ENV{PATH}")
        endif()
    endif()
endmacro()
...
function(vcpkg_configure_make)
    ...
    if(VCPKG_TARGET_IS_ANDROID)
        ...
        if ($ENV{VCPKG_ANDROID_NATIVE_API_LEVEL} MATCHES "^[1-3][0-9]$")
            set(ANDROID_NATIVE_API_LEVEL $ENV{VCPKG_ANDROID_NATIVE_API_LEVEL})
        else()
            set(ANDROID_NATIVE_API_LEVEL ${VCPKG_DETECTED_CMAKE_SYSTEM_VERSION})
        endif()
        ...
        if(TARGET_ARCH MATCHES "armv7-a")
            string(PREPEND VCPKG_DETECTED_CMAKE_C_FLAGS_DEBUG "--target=armv7a-linux-androideabi${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_C_FLAGS_RELEASE "--target=armv7a-linux-androideabi${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG "--target=armv7a-linux-androideabi${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE "--target=armv7a-linux-androideabi${ANDROID_NATIVE_API_LEVEL} ")
        else()
            string(PREPEND VCPKG_DETECTED_CMAKE_C_FLAGS_DEBUG "--target=${TARGET_ARCH}-linux-android${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_C_FLAGS_RELEASE "--target=${TARGET_ARCH}-linux-android${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG "--target=${TARGET_ARCH}-linux-android${ANDROID_NATIVE_API_LEVEL} ")
            string(PREPEND VCPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE "--target=${TARGET_ARCH}-linux-android${ANDROID_NATIVE_API_LEVEL} ")
        endif()
    endif()
