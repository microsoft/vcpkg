...
function(vcpkg_configure_make)
    ...
    if (VCPKG_TARGET_IS_WINDOWS)
        ...
        if(VCPKG_DETECTED_CMAKE_LINKER)
            z_vcpkg_append_to_configure_environment(configure_env LD "${VCPKG_DETECTED_CMAKE_LINKER}")
        endif()
        if(VCPKG_DETECTED_CMAKE_RANLIB)
            z_vcpkg_append_to_configure_environment(configure_env RANLIB "${VCPKG_DETECTED_CMAKE_RANLIB}")
        endif()
        if(VCPKG_DETECTED_CMAKE_OBJDUMP)
            z_vcpkg_append_to_configure_environment(configure_env OBJDUMP "${VCPKG_DETECTED_CMAKE_OBJDUMP}")
        endif()
        ...
        if(VCPKG_DETECTED_CMAKE_DLLTOOL)
            z_vcpkg_append_to_configure_environment(configure_env DLLTOOL "${VCPKG_DETECTED_CMAKE_DLLTOOL}")
        endif()
        foreach(_env IN LISTS arg_CONFIGURE_ENVIRONMENT_VARIABLES)
            z_vcpkg_append_to_configure_environment(configure_env ${_env} "${${_env}}")
    ...
    foreach(current_buildtype IN LISTS all_buildtypes)
        ...
        if (NOT arg_SKIP_CONFIGURE)
            ...
            if(EXISTS "${target_dir}/config.log")
                file(COPY_FILE "${target_dir}/config.log" "${CURRENT_BUILDTREES_DIR}/config.log-${TARGET_TRIPLET}-${short_name_${current_buildtype}}.log")
            endif()
        endif()
