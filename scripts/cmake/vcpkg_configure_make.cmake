...
function(vcpkg_configure_make)
    ...
    set(ENV{INCLUDE} "${VCTOOLDIR}/include")
    ...
    if (CMAKE_HOST_WIN32)
        ...
        if (NOT _csc_DISABLE_AUTO_HOST)
            ...
            if(VCPKG_TARGET_ARCHITECTURE STREQUAL x86)
                set(BUILD_TARGET --target==i686) #${MINGW_TARGET})
                set(HOST_TYPE --host=${MINGW_TARGET})
            elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL x64)
                set(BUILD_TARGET --target==x64-windows) #${MINGW_TARGET})
                set(HOST_TYPE --host=x64-windows)
        ...
        set(COMPILER_CC "CC=cccl CXX=cccl LD=cccl")
        ...
        set(BASH "${MSYS_ROOT}/usr/bin/sh.exe")
...
#INCLUDE=/C/Program\\ Files\\ \\(x86\\)/Microsoft\\ Visual\\ Studio/2019/Community/VC/Tools/MSVC/14.24.28314/include
        file(TO_CMAKE_PATH "$ENV{WindowsSdkDir}" WINSDKDIR)
        string(REGEX REPLACE "([A-Z]):" "/\\1" WINSDKDIR "${WINSDKDIR}")
        string(REPLACE " " "\\\\ " WINSDKDIR "${WINSDKDIR}")
        string(REPLACE "(" "\\\\(" WINSDKDIR "${WINSDKDIR}")
        string(REPLACE ")" "\\\\)" WINSDKDIR "${WINSDKDIR}")
        message(STATUS "${WINSDKDIR}")
        file(TO_CMAKE_PATH "$ENV{VCToolsInstallDir}" VCTOOLDIR)
        string(REGEX REPLACE "([A-Z]):" "/\\1" VCTOOLDIR "${VCTOOLDIR}")
        string(REPLACE " " "\\\\ " VCTOOLDIR "${VCTOOLDIR}")
        string(REPLACE "(" "\\\\(" VCTOOLDIR "${VCTOOLDIR}")
        string(REPLACE ")" "\\\\)" VCTOOLDIR "${VCTOOLDIR}")
        message(STATUS "${VCTOOLDIR}")

        file(TO_CMAKE_PATH "$ENV{WindowsSDKVersion}" WINSDKVER)
        set(UCRTINCLUDE "${WINSDKDIR}/Include/${WINSDKVER}/ucrt")
#INCLUDE=${VCTOOLDIR}/include INCLUDE+=${UCRTINCLUDE}
        set(rel_command
            ${base_cmd} -c "set -a ${COMPILER_CC} echo $INCLUDE ./../${RELATIVE_BUILD_PATH}/configure \"${BUILD_TARGET} ${HOST_TYPE} ${_csc_OPTIONS} ${_csc_OPTIONS_RELEASE}\"")
        set(dbg_command
            ${base_cmd} -c "set -a ${COMPILER_CC} echo $INCLUDE ./../${RELATIVE_BUILD_PATH}/configure \"${BUILD_TARGET} ${HOST_TYPE} ${_csc_OPTIONS} ${_csc_OPTIONS_DEBUG}\"")
    ...
    if(NOT VCPKG_TARGET_IS_WINDOWS)
        ...
        # foreach(_inc_path ${INCLUDE_BACKUP})
            # string(APPEND C_FLAGS_GLOBAL " /I\"${_inc_path}\"")
            # string(APPEND CXX_FLAGS_GLOBAL " /I\"${_inc_path}\"")
        # endforeach()
        #/I$(VCToolsInstallDir)include /I$(WindowsSdkDir)/Include/$(WindowsSDKVersion)ucrt
        string(APPEND C_FLAGS_GLOBAL " --cccl-verbose /showIncludes ")
        string(APPEND CXX_FLAGS_GLOBAL " --cccl-verbose /showIncludes ")
    endif()
