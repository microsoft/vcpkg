...
function(vcpkg_configure_make)
    ...
    if(VCPKG_TARGET_IS_ANDROID)
        ...
        if ($ENV{VCPKG_ANDROID_NATIVE_API_LEVEL} MATCHES "^[1-3][0-9]$")
            set(ANDROID_API_LEVEL $ENV{VCPKG_ANDROID_NATIVE_API_LEVEL})
        else()
            set(ANDROID_API_LEVEL ${VCPKG_DETECTED_CMAKE_SYSTEM_VERSION})
        endif()
        ...
        if(TARGET_ARCH MATCHES "armv7-a")
            set(CFLAGS_target "--target=armv7a-linux-androideabi${ANDROID_API_LEVEL}")
            set(CXXFLAGS_target "--target=armv7a-linux-androideabi${ANDROID_API_LEVEL}")
        else()
            set(CFLAGS_target "--target=${TARGET_ARCH}-linux-android${ANDROID_API_LEVEL}")
            set(CXXFLAGS_target "--target=${TARGET_ARCH}-linux-android${ANDROID_API_LEVEL}")
        endif()
    endif()
    ...
    foreach(current_buildtype IN LISTS all_buildtypes)
        ...
        set(ENV{CFLAGS} "${CFLAGS_target} ${CFLAGS_${current_buildtype}}")
        set(ENV{CXXFLAGS} "${CXXFLAGS_target} ${CXXFLAGS_${current_buildtype}}")
