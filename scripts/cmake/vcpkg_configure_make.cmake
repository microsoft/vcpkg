...
function(vcpkg_configure_make)
    ...
    cmake_parse_arguments(PARSE_ARGV 0 _csc
        ...
        "SOURCE_PATH;PROJECT_SUBPATH;PRERUN_SHELL;BUILD_TRIPLET;CONFIGURE_FILE"
    ...
    if(NOT _csc_CONFIGURE_FILE)
        set(_csc_CONFIGURE_FILE "configure")
    endif()

    if(EXISTS "${SRC_DIR}/${_csc_CONFIGURE_FILE}" AND "${SRC_DIR}/configure.ac") # remove configure; rerun autoconf
        if(NOT VCPKG_MAINTAINER_SKIP_AUTOCONFIG) # If fixing bugs skipping autoconfig saves a lot of time
            ...
            file(REMOVE "${SRC_DIR}/${_csc_CONFIGURE_FILE}") # remove possible autodated configure scripts
            set(_csc_AUTOCONFIG ON)
        endif()
    elseif(EXISTS "${SRC_DIR}/${_csc_CONFIGURE_FILE}" AND NOT _csc_SKIP_CONFIGURE) # run normally; no autoconf or autgen required
    ...
    foreach(_buildtype IN LISTS _buildtypes)
        ...
        if(VCPKG_TARGET_IS_WINDOWS)
            set(command "${base_cmd}" -c "${CONFIGURE_ENV} ./${RELATIVE_BUILD_PATH}/${_csc_CONFIGURE_FILE} ${_csc_BUILD_TRIPLET} ${_csc_OPTIONS} ${_csc_OPTIONS_${_buildtype}}")
        else()
            set(command "${base_cmd}" "./${RELATIVE_BUILD_PATH}/${_csc_CONFIGURE_FILE}" ${_csc_BUILD_TRIPLET} ${_csc_OPTIONS} ${_csc_OPTIONS_${_buildtype}})
        endif()
