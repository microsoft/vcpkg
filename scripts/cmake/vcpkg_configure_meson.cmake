...
function(z_vcpkg_internal_meson_generate_native_file additional_binaries) #https://mesonbuild.com/Native-environments.html
    ...
    foreach(prog IN LISTS proglist)
        if(VCPKG_DETECTED_CMAKE_${prog})
            ...
            string(APPEND native_log "${proglower} = '${VCPKG_DETECTED_CMAKE_${prog}}'\n")
        endif()
    endforeach()
    set(compiler_flags C CXX RC)
    foreach(prog IN LISTS compiler_flags)
        if(VCPKG_DETECTED_CMAKE_${prog}_COMPILER)
            ...
            string(APPEND native_log "${proglower} = '${VCPKG_DETECTED_CMAKE_${prog}_COMPILER}'\n")
        endif()
    endforeach()
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "^(GNU|Intel)$") # for gcc and icc the linker flag -fuse-ld is used. See https://github.com/mesonbuild/meson/issues/8647#issuecomment-878673456
            string(APPEND native_log "c_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        endif()
    endif()
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID MATCHES "^(GNU|Intel)$") # for gcc and icc the linker flag -fuse-ld is used. See https://github.com/mesonbuild/meson/issues/8647#issuecomment-878673456
            string(APPEND native_log "cpp_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        endif()
    endif()
    string(APPEND native_log "cmake = '${CMAKE_COMMAND}'\n")
    foreach(additional_binary IN LISTS additional_binaries)
        string(APPEND native_log "${additional_binary}\n")
    endforeach()

    string(APPEND native_log "[built-in options]\n") #https://mesonbuild.com/Builtin-options.html
    ...
    if(VCPKG_TARGET_IS_WINDOWS)
        ...
        vcpkg_list(TRANSFORM WIN_C_STANDARD_LIBRARIES APPEND "'")
        vcpkg_list(TRANSFORM WIN_C_STANDARD_LIBRARIES PREPEND "'")
        vcpkg_list(REMOVE_ITEM WIN_C_STANDARD_LIBRARIES "''")
        vcpkg_list(JOIN WIN_C_STANDARD_LIBRARIES ", " WIN_C_STANDARD_LIBRARIES)
        string(APPEND native_log "c_winlibs = [${WIN_C_STANDARD_LIBRARIES}]\n")
        ...
        vcpkg_list(TRANSFORM WIN_CXX_STANDARD_LIBRARIES APPEND "'")
        vcpkg_list(TRANSFORM WIN_CXX_STANDARD_LIBRARIES PREPEND "'")
        vcpkg_list(REMOVE_ITEM WIN_CXX_STANDARD_LIBRARIES "''")
        vcpkg_list(JOIN WIN_CXX_STANDARD_LIBRARIES ", " WIN_CXX_STANDARD_LIBRARIES)
        string(APPEND native_log "cpp_winlibs = [${WIN_CXX_STANDARD_LIBRARIES}]\n")
    endif()

    set(native_log_name "${CURRENT_BUILDTREES_DIR}/meson-nativ-${TARGET_TRIPLET}.log")
    set(VCPKG_MESON_NATIVE_FILE "${native_log_name}" PARENT_SCOPE)
    file(WRITE "${native_log_name}" "${native_log}")
endfunction()
...
function(z_vcpkg_internal_meson_convert_compiler_flags_to_list out_var _compiler_flags)
    ...
    vcpkg_list(TRANSFORM ${out_var} STRIP) # Strip leading trailing whitespaces from each element in the list.
    set(${out_var} "${${out_var}}" PARENT_SCOPE)
endfunction()

function(z_vcpkg_internal_meson_convert_list_to_python_array out_var)
    set(flag_list ${ARGN})
    vcpkg_list(TRANSFORM flag_list APPEND "'")
    vcpkg_list(TRANSFORM flag_list PREPEND "'")
...
function(z_vcpkg_internal_meson_generate_native_file_config config_type) #https://mesonbuild.com/Native-environments.html
    ...
    set(native_log_name "${CURRENT_BUILDTREES_DIR}/meson-nativ-${TARGET_TRIPLET}-${lowerconfig}.log")
    set(VCPKG_MESON_NATIVE_FILE_${config_type} "${native_log_name}" PARENT_SCOPE)
    file(WRITE "${native_log_name}" "${native_${config_type}}")
endfunction()
...
function(vcpkg_internal_meson_generate_cross_file additional_binaries) #https://mesonbuild.com/Cross-compilation.html
    ...
    foreach(prog IN LISTS proglist)
        if(VCPKG_DETECTED_CMAKE_${prog})
            string(TOLOWER "${prog}" proglower)
            string(APPEND cross_log "${proglower} = '${VCPKG_DETECTED_CMAKE_${prog}}'\n")
        endif()
    endforeach()
    set(compiler_flags C CXX RC)
    foreach(prog IN LISTS compiler_flags)
        if(VCPKG_DETECTED_CMAKE_${prog}_COMPILER)
            ...
            string(REPLACE "RC" "windres" mesonprog "${mesonprog}") # https://mesonbuild.com/Windows-module.html
            string(TOLOWER "${mesonprog}" proglower)
            string(APPEND cross_log "${proglower} = '${VCPKG_DETECTED_CMAKE_${prog}_COMPILER}'\n")
        endif()
    endforeach()
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "^(GNU|Intel)$") # for gcc and icc the linker flag -fuse-ld is used. See https://github.com/mesonbuild/meson/issues/8647#issuecomment-878673456
            string(APPEND cross_log "c_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        endif()
    endif()
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_CXX_COMPILER_ID MATCHES "^(GNU|Intel)$") # for gcc and icc the linker flag -fuse-ld is used. See https://github.com/mesonbuild/meson/issues/8647#issuecomment-878673456
            string(APPEND cross_log "cpp_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        endif()
    endif()
    string(APPEND cross_log "cmake = '${CMAKE_COMMAND}'\n")
    foreach(additional_binary IN LISTS additional_binaries)
        string(APPEND cross_log "${additional_binary}\n")
    endforeach()

    string(APPEND cross_log "[properties]\n")

    string(APPEND cross_log "[host_machine]\n")
    string(APPEND cross_log "endian = 'little'\n")
    ...
    string(APPEND cross_log "system = '${meson_system_name}'\n")
    string(APPEND cross_log "cpu_family = '${HOST_CPU_FAM}'\n")
    string(APPEND cross_log "cpu = '${HOST_CPU}'\n")

    string(APPEND cross_log "[build_machine]\n")
    string(APPEND cross_log "endian = 'little'\n")
    if(WIN32)
        string(APPEND cross_log "system = 'windows'\n")
    elseif(DARWIN)
        string(APPEND cross_log "system = 'darwin'\n")
    else()
        string(APPEND cross_log "system = 'linux'\n")
    endif()

    if(DEFINED BUILD_CPU_FAM)
        string(APPEND cross_log "cpu_family = '${BUILD_CPU_FAM}'\n")
    endif()
    if(DEFINED BUILD_CPU)
        string(APPEND cross_log "cpu = '${BUILD_CPU}'\n")
    endif()
    ...
    if(NOT BUILD_CPU_FAM MATCHES "${HOST_CPU_FAM}" OR VCPKG_TARGET_IS_ANDROID OR VCPKG_TARGET_IS_IOS OR VCPKG_TARGET_IS_UWP OR (VCPKG_TARGET_IS_MINGW AND NOT WIN32))
        set(native_log_name "${CURRENT_BUILDTREES_DIR}/meson-cross-${TARGET_TRIPLET}.log")
        set(VCPKG_MESON_CROSS_FILE "${native_log_name}" PARENT_SCOPE)
        file(WRITE "${native_log_name}" "${cross_log}")
    endif()
endfunction()
...
function(vcpkg_internal_meson_generate_cross_file_config config_type) #https://mesonbuild.com/Native-environments.html
    ...
    string(TOLOWER "${config_type}" lowerconfig)
    set(native_log_name "${CURRENT_BUILDTREES_DIR}/meson-cross-${TARGET_TRIPLET}-${lowerconfig}.log")
    set(VCPKG_MESON_CROSS_FILE_${config_type} "${native_log_name}" PARENT_SCOPE)
    file(WRITE "${native_log_name}" "${cross_${config_type}_log}")
endfunction()
