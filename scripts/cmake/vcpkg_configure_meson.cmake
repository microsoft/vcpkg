...
function(vcpkg_internal_meson_generate_native_file _additional_binaries) #https://mesonbuild.com/Native-environments.html
    ...
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_C_COMPILER MATCHES "gcc|icc")
            string(APPEND NATIVE "c_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        elseif (NOT VCPKG_DETECTED_CMAKE_C_COMPILER STREQUAL VCPKG_DETECTED_CMAKE_LINKER)
            message(FATAL_ERROR "The linker and compiler executeable must be the same for gcc and icc. Use -fuse-ld=... to change the linker backend.")
        endif()
    endif()
    if(VCPKG_DETECTED_CMAKE_LINKER AND VCPKG_TARGET_IS_WINDOWS)
        if (NOT VCPKG_DETECTED_CMAKE_CXX_COMPILER MATCHES [[g\+\+|icc]])
            string(APPEND NATIVE "cpp_ld = '${VCPKG_DETECTED_CMAKE_LINKER}'\n")
        elseif (NOT VCPKG_DETECTED_CMAKE_CXX_COMPILER STREQUAL VCPKG_DETECTED_CMAKE_LINKER)
            message(FATAL_ERROR "The linker and compiler executeable must be the same for gcc and icc. Use -fuse-ld=... to change the linker backend.")
        endif()
    endif()
    ...
    if(VCPKG_TARGET_IS_WINDOWS)
        ...
        list(FILTER WIN_C_STANDARD_LIBRARIES EXCLUDE REGEX "^''$")
        ...
        list(FILTER WIN_CXX_STANDARD_LIBRARIES EXCLUDE REGEX "^''$")
