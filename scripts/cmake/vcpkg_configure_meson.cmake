...
function(z_meson_generate_native_file additional_binaries) #https://mesonbuild.com/Native-environments.html
...
function(z_meson_generate_flags_properties_string out_var config_type)
    ...
    z_meson_convert_compiler_flags_to_list(MESON_CFLAGS_${config_type} "${VCPKG_DETECTED_CMAKE_C_FLAGS_${config_type}}")
    vcpkg_list(APPEND MESON_CFLAGS_${config_type} "-I${CURRENT_INSTALLED_DIR}/include")
    z_meson_convert_list_to_python_array(MESON_CFLAGS_${config_type} ${MESON_CFLAGS_${config_type}})
    string(APPEND ${out_var} "c_args = ${MESON_CFLAGS_${config_type}}\n")
    z_meson_convert_compiler_flags_to_list(MESON_CXXFLAGS_${config_type} "${VCPKG_DETECTED_CMAKE_CXX_FLAGS_${config_type}}")
    vcpkg_list(APPEND MESON_CXXFLAGS_${config_type} "-I${CURRENT_INSTALLED_DIR}/include")
    z_meson_convert_list_to_python_array(MESON_CXXFLAGS_${config_type} ${MESON_CXXFLAGS_${config_type}})
    ...
    z_meson_convert_compiler_flags_to_list(LINKER_FLAGS_${config_type} "${LINKER_FLAGS_${config_type}}")
    vcpkg_list(APPEND LINKER_FLAGS_${config_type} "${LIBPATH_${config_type}}")
    z_meson_convert_list_to_python_array(LINKER_FLAGS_${config_type} ${LINKER_FLAGS_${config_type}})
    string(APPEND ${out_var} "c_link_args = ${LINKER_FLAGS_${config_type}}\n")
    string(APPEND ${out_var} "cpp_link_args = ${LINKER_FLAGS_${config_type}}\n")
    set(${out_var} "${${out_var}}" PARENT_SCOPE)
endfunction()

function(z_meson_generate_native_file_config config_type) #https://mesonbuild.com/Native-environments.html
    set(native_${config_type} "[properties]\n") #https://mesonbuild.com/Builtin-options.html
    z_meson_generate_flags_properties_string(native_properties ${config_type})
...
function(z_meson_generate_cross_file additional_binaries) #https://mesonbuild.com/Cross-compilation.html
    if(CMAKE_HOST_WIN32)
...
function(z_meson_generate_cross_file_config config_type) #https://mesonbuild.com/Native-environments.html
    set(cross_${config_type}_log "[properties]\n") #https://mesonbuild.com/Builtin-options.html
    z_meson_generate_flags_properties_string(cross_properties ${config_type})
...
function(vcpkg_configure_meson)
    ...
    if(NOT VCPKG_MESON_CROSS_FILE)
        z_meson_generate_cross_file("${arg_ADDITIONAL_CROSS_BINARIES}")
    endif()
    ...
    if(NOT VCPKG_MESON_CROSS_FILE_DEBUG AND VCPKG_MESON_CROSS_FILE)
        z_meson_generate_cross_file_config(DEBUG)
    endif()
    if(NOT VCPKG_MESON_CROSS_FILE_RELEASE AND VCPKG_MESON_CROSS_FILE)
        z_meson_generate_cross_file_config(RELEASE)
    endif()
    ...
    if(NOT VCPKG_MESON_NATIVE_FILE AND NOT VCPKG_MESON_CROSS_FILE)
        z_meson_generate_native_file("${arg_ADDITIONAL_NATIVE_BINARIES}")
    endif()
    if(NOT VCPKG_MESON_NATIVE_FILE_DEBUG AND NOT VCPKG_MESON_CROSS_FILE)
        z_meson_generate_native_file_config(DEBUG)
    endif()
    if(NOT VCPKG_MESON_NATIVE_FILE_RELEASE AND NOT VCPKG_MESON_CROSS_FILE)
        z_meson_generate_native_file_config(RELEASE)
    endif()
