function(vcpkg_from_github)
    ...
    unset(fully_qualified_ref)
    if(DEFINED arg_REF)
        set(_regex_hex_char "[0-9a-fA-F]")
        string(REPEAT "${_regex_hex_char}" 40 _regex_hex40)
        if(arg_REF MATCHES "^${_regex_hex40}\$")
            set(fully_qualified_ref "${arg_REF}")
        elseif(arg_REF MATCHES "^refs/")
            message(FATAL_ERROR "Ambiguous git refs, the REF argument should not start with 'refs/'.")
        else()
            set(fully_qualified_ref "refs/tags/${arg_REF}")
        endif()
    endif()

    if(DEFINED fully_qualified_ref AND NOT DEFINED arg_SHA512)
        message(FATAL_ERROR "SHA512 must be specified if REF is specified.")
    endif()
    if(NOT DEFINED fully_qualified_ref AND DEFINED arg_SHA512)
    ...
    if(NOT DEFINED fully_qualified_ref AND NOT DEFINED arg_HEAD_REF)
    ...
    elseif(NOT VCPKG_USE_HEAD_VERSION AND NOT DEFINED fully_qualified_ref)
...
")
        ...
        set(ref_to_use "${fully_qualified_ref}")
    ...
    string(REGEX REPLACE "^refs/tags/" "" sanitized_ref "${ref_to_use}")
    string(REPLACE "/" "_-" sanitized_ref "${sanitized_ref}")
