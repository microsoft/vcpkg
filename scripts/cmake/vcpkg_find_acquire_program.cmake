...
function(vcpkg_find_acquire_program VAR)
  ...
  elseif(VAR MATCHES "MESON")
    ...
    set(PATHS ${DOWNLOADS}/tools/meson/meson-${VERSION})
    set(_vfa_SUPPORTED ON)
    set(_vfa_ONLY_FIND_INTERNAL ON)
    set(ARCHIVE "meson-${VERSION}.tar.gz")
    set(HASH bf2a5bab19deb433e363bf569527dbd1a9e8c0f77482aa077abbaa480dadc396e43148957473a31ecd9d3965578d2f7b7dc8808839c619ae2c0d419aed72d4e2)
  ...
  macro(do_find)
    if(NOT DEFINED REQUIRED_INTERPRETER)
      ...
      if(REQUIRED_PYTHON_PACKAGES AND REQUIRED_INTERPRETER MATCHES "PYTHON3")
        get_filename_component(PYTHON_DIR "${${REQUIRED_INTERPRETER}}" DIRECTORY)
        set(PIP_VER)
        if(REQUIRED_INTERPRETER MATCHES "PYTHON3")
            set(PIP_VER 3)
        endif()
        get_filename_component(PYTHON_DIR "${${REQUIRED_INTERPRETER}}" DIRECTORY)
        find_program(PIP NAMES pip${VCPKG_HOST_EXECUTABLE_SUFFIX} pip${PIP_VER}${VCPKG_HOST_EXECUTABLE_SUFFIX} PATHS "${PYTHON_DIR}/Scripts" "${PYTHON_DIR}")
        if(NOT PIP)
            message(FATAL_ERROR "Unable to find pip on PATHS '${PYTHON_DIR}/Scripts' '${PYTHON_DIR}'")
        endif()
        set(PYTHON_INSTALL_COMMAND ${PIP} install ${REQUIRED_PYTHON_PACKAGES} ${PYTHON_OPTION})
  ...
  if(VAR MATCHES "PYTHON3")
    set(PIP_VER 3)
    get_filename_component(PYTHON_DIR "${${VAR}}" DIRECTORY)
    find_program(PIP NAMES pip${PIP_VER}${VCPKG_HOST_EXECUTABLE_SUFFIX} PATHS "${PYTHON_DIR}/Scripts" "${PYTHON_DIR}")
    if(NOT PIP AND PYTHON_DIR MATCHES "${DOWNLOADS}")
        vcpkg_from_github(
