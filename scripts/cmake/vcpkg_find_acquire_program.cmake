function(z_vcpkg_find_acquire_program_version_check out_var)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "EXACT_VERSION_MATCH"
        "MIN_VERSION;PROGRAM_NAME;VERSION_REGEX"
        "COMMAND"
    )
    ...
    if(NOT "${arg_VERSION_REGEX}" STREQUAL "")
        # parse version number
        STRING(REGEX MATCH "${arg_VERSION_REGEX}" program_version_output ${program_version_output})
    endif()
...
function(z_vcpkg_find_acquire_program_find_external program)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "EXACT_VERSION_MATCH"
        "INTERPRETER;MIN_VERSION;PROGRAM_NAME;VERSION_REGEX"
    ...
    elseif(${program}) # only do a version check if ${program} has a value
        z_vcpkg_find_acquire_program_version_check(version_is_good
            ...
            VERSION_REGEX ${arg_VERSION_REGEX}
...
function(vcpkg_find_acquire_program program)
    ...
    elseif(program STREQUAL "CLANG7")
        set(program_name clang)
        set(program_version 7.0.0)
        set(version_command --version)
        set(version_regex "[0-9]+\\.[0-9]+\\.[0-9]+")
        set(extra_search_args EXACT_VERSION_MATCH)
        if(EXISTS "${CURRENT_HOST_INSTALLED_DIR}/share/vcpkg-tool-clang7/vcpkg-port-config.cmake")
            set(paths_to_search "${CURRENT_HOST_INSTALLED_DIR}/tools/vcpkg-tool-clang7")
        else() # Old behavior
            message("Consider adding vcpkg-tool-clang7 as a host dependency of this port or create an issue at https://github.com/microsoft/vcpkg/issues")
            if(CMAKE_HOST_WIN32)
                if(DEFINED ENV{PROCESSOR_ARCHITEW6432})
                    set(host_arch "$ENV{PROCESSOR_ARCHITEW6432}")
                else()
                ...
                if(host_arch MATCHES "64")
                    set(tool_subdirectory "clang-${program_version}-win64")
                    set(download_urls "https://releases.llvm.org/${program_version}/LLVM-${program_version}-win64.exe")
                    set(download_filename "LLVM-${program_version}-win64.7z.exe")
                    set(download_sha512 c2b1342469275279f833fdc1e17ba5a9f99021306d6ab3d7209822a01d690767739eebf92fd9f23a44de5c5d00260fed50d5262b23a8eccac55b8ae901e2815c)
                else()
                    set(tool_subdirectory "clang-${program_version}-win32")
                    set(download_urls "https://releases.llvm.org/${program_version}/LLVM-${program_version}-win32.exe")
                    set(download_filename "LLVM-${program_version}-win32.7z.exe")
                    set(download_sha512 4a9539beb9792ecdf30ab65db5f63aa3b0233d1268e25eb6fcd944684f44d79647fe7444eff66741d0c934a0736ea6235796cd6bc42386641147e648bbe52124)
                endif()
            ...
            elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "Linux")
                ...
                EXEC_PROGRAM(uname ARGS -m OUTPUT_VARIABLE HOST_ARCH)
                if(HOST_ARCH STREQUAL "aarch64")
                    set(basename "clang+llvm-${program_version}-aarch64-linux-gnu")
                    set(download_sha512 73a52bdc9d3aad58812623b4af719dd8c628eb66f7ffdd79798456222d86c2cc13e264136e0ccbf54466f9897fa7239da7fbeb9fd4836cb5a2ece79123b23dd4)
                else()
                    set(basename "clang+llvm-${program_version}-x86_64-linux-gnu-ubuntu-16.04")
                    set(download_sha512 fb3dc588137426dc28a20ef5e34e9341b18114f03bf7d83fafbb301efbfd801bba08615b804817c80252e366de9d2f8efbef034e53a1b885b34c86c2fbbf9c28)
                endif()
                set(tool_subdirectory "${basename}")
                set(download_urls "https://releases.llvm.org/${program_version}/${basename}.tar.xz")
                set(download_filename "${basename}.tar.xz")
                set(paths_to_search "${DOWNLOADS}/tools/${program_name}/${tool_subdirectory}/${tool_subdirectory}/bin")
            elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "FreeBSD")
                set(supported_on_unix ON)
                EXEC_PROGRAM(uname ARGS -m OUTPUT_VARIABLE HOST_ARCH)
                if(HOST_ARCH STREQUAL "amd64")
                    set(basename "clang+llvm-${program_version}-amd64-unknown-freebsd11")
                    set(download_sha512 d501484c38cfced196128866a19f7fef1e0b5d609ea050d085b7deab04ac8cc2bbf74b3cfe6cd90d8ea17a1d9cfca028a6c933f0736153ba48785ddc8646574f)
                else()
                    set(basename "clang+llvm-${program_version}-i386-unknown-freebsd11")
                    set(download_sha512 cb973bec29a2386c78233c5e1df759985c15056eba74653c4066ee932e8fcd56eee6467492cff2e283bf87363f12613899c566af6fcb0fcd35423c08ecb8f47d)
                endif()
                set(tool_subdirectory "${basename}")
                set(download_urls "https://releases.llvm.org/${program_version}/${basename}.tar.xz")
                set(download_filename "${basename}.tar.xz")
            ...
            elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
                set(supported_on_unix ON)
                EXEC_PROGRAM(arch OUTPUT_VARIABLE HOST_ARCH)
                if(HOST_ARCH STREQUAL "arm64")
                    message(WARNING "No prebuilt binaries for ${program_version} at version ${program_version}, falling back on search on user-system for a match.")
                    set(supported_on_unix OFF)
                    set(brew_package_name "llvm@7") # brew package exists but incorrect as it points to LLVM 7.1.0 and not 7.0.0
                    set(apt_package_name "clang-7")
                else()
                    set(supported_on_unix ON)
                    set(basename "clang+llvm-${program_version}-x86_64-apple-darwin")
                    set(download_sha512 c5ca6a7756e0cecdf78d4d0c522fe7e803d4b1b2049cb502a034fe8f5ca30fcbf0e738ebfbc89c87de8adcd90ea64f637eb82e9130bb846b43b91f67dfa4b916)
                    set(tool_subdirectory "${basename}")
                    set(download_urls "https://releases.llvm.org/${program_version}/${basename}.tar.xz")
                    set(download_filename "${basename}.tar.xz")
                endif()
