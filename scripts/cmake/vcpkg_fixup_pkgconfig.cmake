...
function(vcpkg_fixup_pkgconfig_check_files pkg_cfg_cmd _file _config _system_libs _ignore_flags)
    ...
    foreach(LIBS_ARG IN LISTS LIBS_ARGS)
        ...
        elseif(LIBS_ARG MATCHES "^-l(.*)")
            ...
            string(REPLACE "+" "p" LIBNAMEVAR "${LIBNAME}")
            string(REPLACE "-" "_" LIBNAMEVAR "${LIBNAMEVAR}")
            # Ensure existance in current packages and installed
            find_library("CHECK_LIB_${LIBNAMEVAR}_${_config}" NAMES "${LIBNAME}" PATHS ${SEARCH_PATHS} "${CURRENT_INSTALLED_DIR}${PATH_SUFFIX_${_config}}/lib" NO_DEFAULT_PATH)
            if("${CHECK_LIB_${LIBNAMEVAR}_${_config}}" MATCHES "-NOTFOUND\$")
                message(FATAL_ERROR "find_library() failed with result: ${CHECK_LIB_${LIBNAMEVAR}_${_config}}\n    find_library(CHECK_LIB_${LIBNAMEVAR}_${_config} NAMES \"${LIBNAME}\" PATHS ${SEARCH_PATHS} \"${CURRENT_INSTALLED_DIR}${PATH_SUFFIX_${_config}}/lib\" NO_DEFAULT_PATH)")
            else()
                debug_message("CHECK_LIB_${LIBNAMEVAR}_${_config}=${CHECK_LIB_${LIBNAMEVAR}_${_config}}")
            endif()
        endif()
    endforeach()
