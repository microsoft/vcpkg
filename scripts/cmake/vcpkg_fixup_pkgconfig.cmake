...
function(vcpkg_fixup_pkgconfig_check_libraries _config _contents_var _system_libs _system_packages)
    ...
    foreach(_lib ${_all_libs})
        ...
        unset(NO_CHECK_LIB)
        message(STATUS "CHECKING: x${_lib}x")
        if("${_lib}" MATCHES "^-L(.+)$")
        ...
        elseif("${_lib}" MATCHES "^-l(.+)$")
            ...
            foreach(_system_lib "${_system_libs}")
                ...
                if("${_libname}" STREQUAL "${_system_lib}")
                    set(NO_CHECK_LIB ON)
            ...
            message(STATUS "Searching for library ${_libname}")
            if(EXISTS "${_libname}") #full path
                set(CHECK_LIB "${_libname}" CACHE INTERNAL FORCE)
            endif()
            find_library(CHECK_LIB NAMES ${_libname} PATHS "${libprefix}")
            message(STATUS "CHECK_LIB: ${CHECK_LIB}")
            if(NOT CHECK_LIB)
                ...
                foreach(_lib_suffix ${lib_suffixes})
                    ...
                    find_library(CHECK_LIB NAMES ${_name_without_extension}${_lib_suffix} PATHS "${libprefix}")
                    if(CHECK_LIB)
                        message(STATUS "Found ${CHECK_LIB} with additional debug suffix! Please correct the *.pc file!")
                        string(REGEX REPLACE "(-l${_name_without_extension})(\.dll\.a|\.a|\.lib|\.so)" "\\1${_lib_suffix}\\2" _contents ${_contents})
                    endif()
                endforeach()
                if(NOT CHECK_LIB)
                    message(STATUS "Library ${_libname} was not found! If it is a system library use the SYSTEM_LIBRARIES parameter for the vcpkg_fixup_pkgconfig call! Otherwise, corret the *.pc file")
                endif()
            endif()
        else()
            message(WARNING "Found ${_lib} and no rule to analyse the flag! Please check the *.pc file")
        endif()
        unset(CHECK_LIB CACHE)
        unset(NO_CHECK_LIB)
    endforeach()
    ...
    string(REGEX REPLACE "(Requires:[\t ]| )" "" _pkg_list_tmp "${_pkg}")
    string(REGEX REPLACE "," ";" _pkg_list_tmp "${_pkg_list_tmp}")
    string(REGEX MATCH "Requires.private:[^\n]+" _pkg_private_list_tmp "${_contents}")
    string(REGEX REPLACE "(Requires:[\t ]| )" "" _pkg_private_list_tmp "${_pkg_private_list_tmp}")
    ...
    foreach(_package  ${_pkg_list_tmp} ${_pkg_private_list_tmp})
        message(STATUS "Searching for packages: ${_package}")
        set(PKG_CHECK ON)
        if(NOT "${_system_packages}" STREQUAL "")
            ...
            if("${_system_packages}" MATCHES "${_package}" )
                message(STATUS "Package ${_package} is SYSTEM PACKAGE!") 
                set(PKG_CHECK OFF)
            endif()
        endif()
        if(PKG_CHECK AND NOT EXISTS "${libprefix}/pkgconfig/${_package}.pc")
            message(STATUS "Package ${_package} not found! If it is a system package add it to the SYSTEM_PACKAGES parameter for the vcpkg_fixup_pkgconfig call! Otherwise, corret the *.pc file")
        else()
            message(STATUS "Found package ${_package}!")
        endif()
    endforeach()
...
set(_file "D:\\xlinux\\installed\\x64-windows\\lib\\pkgconfig\\bzip2.pc")
include(${CMAKE_CURRENT_LIST_DIR}/vcpkg_common_definitions.cmake)
file(READ "${_file}" _contents)
set(CURRENT_INSTALLED_DIR "D:\\xlinux\\installed\\x64-windows\\")
