...
def get_current_git_ref():
    output = subprocess.run(
        ["git", "-C", SCRIPT_DIRECTORY, "rev-parse", "--verify", "HEAD"],
        capture_output=True,
        encoding="utf-8",
    )
...
def generate_versions_db(revision):
    ...
    port_names = [
        item
        for item in os.listdir(PORTS_DIRECTORY)
        if os.path.isdir(os.path.join(PORTS_DIRECTORY, item))
    ]
    ...
    for i, _ in enumerate(
        process_pool.imap_unordered(generate_versions_file, port_names), 1
    ):
