        ...
        if (!manifest.empty())
        {
            ...
            if (ec)
            {
                Checks::exit_with_message(
                    VCPKG_LINE_INFO, "Failed to read manifest file %s: %s", fs::u8string(manifest), ec.message());
            }
            ...
            else
            {
                ...
                Checks::exit_with_message(
                    VCPKG_LINE_INFO, "Error: Failed to load manifest file %s.", fs::u8string(manifest));
            }
        }
        else
        {
