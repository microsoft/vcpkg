...
using vcpkg::DigestAlgorithm;
using vcpkg::DigestHasher;
using vcpkg::Optional;
using vcpkg::StringView;

namespace Checks = vcpkg::Checks;
namespace Strings = vcpkg::Strings;

using uchar = unsigned char;

Optional<DigestAlgorithm> vcpkg::digest_algorithm_from_string(StringView sv) noexcept
{
...
const char* vcpkg::to_string(DigestAlgorithm algo) noexcept
{
    switch (algo)
    {
...
namespace
{
    ...
    struct BCryptHasher : DigestHasher
    {
        ...
        explicit BCryptHasher(DigestAlgorithm algo) noexcept
        {
            switch (algo)
            {
                case DigestAlgorithm::Sha1: alg_handle = sha1_alg_handle; break;
                case DigestAlgorithm::Sha256: alg_handle = sha256_alg_handle; break;
                case DigestAlgorithm::Sha512: alg_handle = sha512_alg_handle; break;
    ...
    struct DigestHasherImpl final : DigestHasher
    {
        DigestHasherImpl() = default;
    ...
    private:
        ...
        using underlying_type = typename DigestAlgorithm::underlying_type;
        using message_length_type = typename DigestAlgorithm::message_length_type;
        constexpr static std::size_t chunk_size = DigestAlgorithm::chunk_size;

        DigestAlgorithm m_impl{};
    ...
    struct Sha512Algorithm
    {
        ...
        using message_length_type = vcpkg::UInt128;
...
std::unique_ptr<DigestHasher> vcpkg::get_hasher_for(DigestAlgorithm algo) noexcept
{
#if defined(_WIN32)
    ...
    switch (algo)
    {
        case DigestAlgorithm::Sha1: return std::make_unique<DigestHasherImpl<Sha1Algorithm>>();
        case DigestAlgorithm::Sha256: return std::make_unique<DigestHasherImpl<Sha256Algorithm>>();
        case DigestAlgorithm::Sha512: return std::make_unique<DigestHasherImpl<Sha512Algorithm>>();
...
template<class F>
static std::string do_hash(DigestAlgorithm algo, const F& f) noexcept
{
#if defined(_WIN32)
    ...
    switch (algo)
    {
        ...
        {
            auto hasher = DigestHasherImpl<Sha1Algorithm>();
            return f(hasher);
        }
        case DigestAlgorithm::Sha256:
        {
            auto hasher = DigestHasherImpl<Sha256Algorithm>();
            return f(hasher);
        }
        case DigestAlgorithm::Sha512:
        {
            auto hasher = DigestHasherImpl<Sha512Algorithm>();
            return f(hasher);
        }
        default: vcpkg::Checks::exit_with_message(VCPKG_LINE_INFO, "Unknown hashing algorithm: %s", algo);
    }
...
std::string vcpkg::get_bytes_digest(const void* first, const void* last, DigestAlgorithm algo) noexcept
{
    return do_hash(algo, [first, last](DigestHasher& hasher) {
        hasher.add_bytes(first, last);
        return hasher.get_hash();
    });
}

std::string vcpkg::get_string_digest(StringView sv, DigestAlgorithm algo) noexcept
{
    return vcpkg::get_bytes_digest(sv.data(), sv.data() + sv.size(), algo);
}

// TODO: use Files::Filesystem to open a file
std::string vcpkg::get_file_digest(const Files::Filesystem&,
                                   const fs::path& path,
                                   ...
                                   std::error_code& ec) noexcept
{
    ...
    return do_hash(algo, [&file, &ec](DigestHasher& hasher) {
