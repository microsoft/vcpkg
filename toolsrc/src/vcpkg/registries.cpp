...
namespace
{
    ...
    static const fs::path port_versions_dir = fs::u8path("port_versions");
        ...
        if (!m_baseline_identifier.empty())
        {
            ...
            if (fs.exists(versions_path))
            {
                auto maybe_version_entries =
                    load_versions_file(fs, VersionDbType::Git, paths.builtin_port_versions, port_name);
                Checks::check_exit(
    ...
    {
        const auto& fs = paths.get_filesystem();
        if (!m_baseline_identifier.empty() && fs.exists(paths.builtin_port_versions))
        {
