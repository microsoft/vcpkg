...
namespace vcpkg::VisualStudio
{
    ...
    static std::vector<VisualStudioInstance> get_visual_studio_instances_internal(const VcpkgPaths& paths)
    {
        ...
        const auto append_if_has_cl_vc140 = [&](fs::path&& path_root) {
        ...
        if (const auto path_as_string = maybe_vs140_comntools.get())
        {
            ...
            append_if_has_cl_vc140(fs::path{*path_as_string}.parent_path().parent_path());
            append_if_has_cl_vc140(fs::path{*path_as_string}.parent_path().parent_path().parent_path());
        }
