...
namespace vcpkg::Dependencies
{
    ...
    /// </param>
    void PackageGraph::install(const FeatureSpec& spec, const std::unordered_set<std::string>& prevent_default_features)
    {
                                              ...
                                              const InstallPlanAction& p)
    {
        ...
        if (auto scf = p.source_control_file.get())
        {
            ...
            const Build::BuildPackageConfig build_config{
                *scf, triplet, paths.port_dir(p.spec), s_build_options, p.feature_list};
