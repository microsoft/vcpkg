            ...
            if (std::regex_match(to_autocomplete, match, std::regex{command.regex.c_str()}))
            {
                ...
                if (is_option)
                {
                    results = Util::fmap(command.structure.options.switches,
                                         [](const CommandSwitch& s) -> std::string { return Strings::format("--%s", s.name.to_string()); });

                    auto settings = Util::fmap(command.structure.options.settings, [](auto&& s) { return Strings::format("--%s", s.name); });
                    results.insert(results.end(), settings.begin(), settings.end());

                    auto multisettings = Util::fmap(command.structure.options.multisettings, [](auto&& s) { return Strings::format("--%s", s.name); });
