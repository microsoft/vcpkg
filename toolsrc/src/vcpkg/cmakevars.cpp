    ...
    const Span<const std::pair<const FullPackageSpec*, std::string>>& spec_abi_settings) const
{
    ...
    constexpr int COMMAND_LENGTH = COMMAND_START.size() + COMMAND_END.size();
    ...
    const int space_quote_chars = spec_abi_settings.size() * 11;
    ...
    for (const auto& spec_abi_setting : spec_abi_settings)
    {
        ...
        Strings::append(extraction_file,
                        ...
                        *feature_list_itr,
                        "\" \"",
                        paths.get_triplet_file_path(spec.package_spec.triplet()),
                        "\" \"",
...
fs::path CMakeVarProvider::create_dep_info_extraction_file(const Span<const PackageSpec> specs) const
{
    ...
    const int space_chars = specs.size();
