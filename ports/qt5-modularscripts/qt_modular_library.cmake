...
function(qt_modular_build_library SOURCE_PATH)
    ...
    if (RELEASE_EXE)
       file(INSTALL ${RELEASE_EXE} DESTINATION ${CURRENT_PACKAGES_DIR}/tools/${PORT})
       file(REMOVE ${RELEASE_EXE})
       foreach(_exe ${RELEASE_EXE})
           string(REPLACE ".exe" ".pdb" _prb_file ${_exe})
           if(EXISTS ${_prb_file})
            file(INSTALL ${_prb_file} DESTINATION ${CURRENT_PACKAGES_DIR}/tools/${PORT})
           endif()
       endforeach()
        # Check if there are any binaries left over; if not - delete the directory
       file(GLOB RELEASE_BINS ${CURRENT_PACKAGES_DIR}/bin/*)
       if(NOT RELEASE_BINS)
           file(REMOVE_RECURSE ${CURRENT_PACKAGES_DIR}/bin)
       endif()
    endif()
    if(DEBUG_EXE)
       file(INSTALL ${DEBUG_EXE} DESTINATION ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT})
       file(REMOVE ${DEBUG_EXE})
       foreach(_exe ${DEBUG_EXE})
           string(REPLACE ".exe" ".pdb" _prb_file ${_exe})
           if(EXISTS ${_prb_file})
            file(INSTALL ${_prb_file} DESTINATION ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT})
           endif()
       endforeach()
        # Check if there are any binaries left over; if not - delete the directory
       file(GLOB RELEASE_BINS ${CURRENT_PACKAGES_DIR}/debug/bin/*)
       if(NOT RELEASE_BINS)
