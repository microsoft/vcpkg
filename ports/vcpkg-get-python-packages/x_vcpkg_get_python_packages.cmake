...
#]===]
if(Z_VCPKG_GET_PYTHON_PACKAGES_GUARD)
    return()
endif()
set(Z_VCPKG_GET_PYTHON_PACKAGES_GUARD ON CACHE INTERNAL "guard variable")

function(x_vcpkg_get_python_package PYTHON_DIR )
    ...
    if("${arg_PYTHON_DIR}" MATCHES "${DOWNLOADS}") # inside vcpkg
        if(NOT EXISTS "${arg_PYTHON_DIR}/easy_install${VCPKG_HOST_EXECUTABLE_SUFFIX}")
            if(NOT EXISTS "${arg_PYTHON_DIR}/Scripts/pip${VCPKG_HOST_EXECUTABLE_SUFFIX}")
                ...
                execute_process(COMMAND "${arg_PYTHON_EXECUTABLE}" "${PYFILE_PATH}/get-pip.py" ${PYTHON_OPTION})
            endif()
            foreach(_package IN LISTS arg_PACKAGES)
                execute_process(COMMAND "${arg_PYTHON_DIR}/Scripts/pip${VCPKG_HOST_EXECUTABLE_SUFFIX}" install ${_package} ${PYTHON_OPTION})
            endforeach()
        else()
            foreach(_package IN LISTS arg_PACKAGES)
                execute_process(COMMAND "${arg_PYTHON_DIR}/easy_install${VCPKG_HOST_EXECUTABLE_SUFFIX}" ${_package})
            endforeach()
        endif()
        if(NOT VCPKG_TARGET_IS_WINDOWS)
            execute_process(COMMAND pip3 install ${arg_PACKAGES})
        endif()
    else() # outside vcpkg
        foreach(_package IN LISTS arg_PACKAGES)
            execute_process(COMMAND ${arg_PYTHON_EXECUTABLE} -c "import ${_package}" RESULT_VARIABLE HAS_ERROR)
            if(HAS_ERROR)
