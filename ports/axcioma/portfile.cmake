...
set(DANCEX11_BASE_PATH ${SOURCE_PATH}/taox11/dancex11)
...
function(install_libraries SOURCE_PATH LIBRARIES)
  ...
  foreach(LIBRARY ${LIBRARIES})
    ...
    if(VCPKG_LIBRARY_LINKAGE STREQUAL dynamic)
      ...
      if(EXISTS ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_RELEASE_SUFFIX})
        if(VCPKG_TARGET_IS_WINDOWS)
          file(COPY ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_RELEASE_SUFFIX} DESTINATION ${CURRENT_PACKAGES_DIR}/bin)
        else()
          vcpkg_execute_required_process(COMMAND cp "${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_RELEASE_SUFFIX}" "${CURRENT_PACKAGES_DIR}/lib/" WORKING_DIRECTORY ${SOURCE_PATH} LOGNAME copy-libs-${VCPKG_BUILD_TYPE}-${TARGET_TRIPLET})
        endif()
      endif()
      if(EXISTS ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_DEBUG_SUFFIX})
        if(VCPKG_TARGET_IS_WINDOWS)
          file(COPY ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_DEBUG_SUFFIX} DESTINATION ${CURRENT_PACKAGES_DIR}/debug/bin)
        else()
          vcpkg_execute_required_process(COMMAND cp "${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_DEBUG_SUFFIX}" "${CURRENT_PACKAGES_DIR}/debug/lib/" WORKING_DIRECTORY ${SOURCE_PATH} LOGNAME copy-libs-${VCPKG_BUILD_TYPE}-${TARGET_TRIPLET})
        endif()
      endif()
    endif()
    ...
    if(EXISTS ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_DECORATOR}${LIB_RELEASE_SUFFIX})
      if(VCPKG_TARGET_IS_WINDOWS)
        ...
        vcpkg_execute_required_process(COMMAND cp "${LIB_PATH}${LIB_PREFIX}${LIBRARY}${DLL_DECORATOR}${LIB_RELEASE_SUFFIX}" "${CURRENT_PACKAGES_DIR}/lib/" WORKING_DIRECTORY ${SOURCE_PATH} LOGNAME copy-libs-${VCPKG_BUILD_TYPE}-${TARGET_TRIPLET})
      endif()
    endif()
    if(EXISTS ${LIB_PATH}/${LIB_PREFIX}${LIBRARY}${DLL_DECORATOR}${LIB_DEBUG_SUFFIX})
      if(VCPKG_TARGET_IS_WINDOWS)
        ...
        vcpkg_execute_required_process(COMMAND cp "${LIB_PATH}${LIB_PREFIX}${LIBRARY}${DLL_DECORATOR}${LIB_DEBUG_SUFFIX}" "${CURRENT_PACKAGES_DIR}/debug/lib/" WORKING_DIRECTORY ${SOURCE_PATH} LOGNAME copy-libs-${VCPKG_BUILD_TYPE}-${TARGET_TRIPLET})
      endif()
    endif()         
  endforeach()
endfunction()
