...
find_path(
  ...
  PATHS $ENV{ACE_ROOT} $ENV{ACE_ROOT}/include /usr/include /usr/local/include
  NO_DEFAULT_PATH)
find_library(
  ACE_LIBRARY_RELEASE
  NAMES ACE
...
if(ACE_FOUND)
  ...
  foreach(LIBRARY ${ACE_FIND_LIBS})
    ...
    if(WIN32)
      find_library(
        ACE_${LIBRARY}_LIBRARY_DEBUG
        NAMES "ACE_${LIBRARY}d" "ACE${LIBRARY}d" "${LIBRARY}d"
        PATHS ${ACE_LIBRARY_DEBUG_DIR}
        NO_DEFAULT_PATH)
      if(ACE_${LIBRARY}_LIBRARY_DEBUG)
        message(
          STATUS "${LIBRARY} debug found at: ${ACE_${LIBRARY}_LIBRARY_DEBUG}")
    ...
    find_library(
      ACE_${LIBRARY}_LIBRARY_RELEASE
      NAMES "ACE_${LIBRARY}" "ACE${LIBRARY}" "${LIBRARY}"
      PATHS ${ACE_LIBRARY_RELEASE_DIR}
      NO_DEFAULT_PATH)
    if(ACE_${LIBRARY}_LIBRARY_RELEASE)
      if(WIN32)
        message(
          STATUS
            "${LIBRARY} release found at: ${ACE_${LIBRARY}_LIBRARY_RELEASE}")
      else(WIN32)
    ...
    if(WIN32)
      set(ACE_${LIBRARY}_LIBRARY optimized ${ACE_${LIBRARY}_LIBRARY_RELEASE}
                                 debug ${ACE_${LIBRARY}_LIBRARY_DEBUG})
    else(WIN32)
