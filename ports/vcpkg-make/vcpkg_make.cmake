...
function(vcpkg_make_setup_win_msys msys_out)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        ""
        ""
        "ADDITIONAL_MSYS_PACKAGES;DIRECT_PACKAGES"
    )
    ...
    if(DEFINED arg_ADDITIONAL_MSYS_PACKAGES)
        list(APPEND msys_require_packages ${arg_ADDITIONAL_MSYS_PACKAGES})
    endif()
    if(DEFINED arg_DIRECT_PACKAGES)
        vcpkg_insert_msys_into_path(msys PACKAGES ${msys_require_packages} DIRECT_PACKAGES ${arg_DIRECT_PACKAGES})
    else()
...
function(vcpkg_make_get_shell out_var)
    ...
    if(CMAKE_HOST_WIN32)
        if(NOT DEFINED VCPKG_MAKE_ACQUIRE_MSYS OR VCPKG_MAKE_ACQUIRE_MSYS)
            z_vcpkg_get_global_property(additional_msys_packages "make_additional_msys_packages")
            z_vcpkg_get_global_property(direct_msys_packages "make_direct_msys_packages")
            
            set(setup_args "")
            if(additional_msys_packages)
                list(APPEND setup_args ADDITIONAL_MSYS_PACKAGES ${additional_msys_packages})
            endif()
            if(direct_msys_packages)
                list(APPEND setup_args DIRECT_PACKAGES ${direct_msys_packages})
            endif()

            vcpkg_make_setup_win_msys(msys_root ${setup_args})
