...
if(VCPKG_TARGET_IS_WINDOWS)
    if (VCPKG_LIBRARY_LINKAGE STREQUAL dynamic)
        if(EXISTS "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-debug.cmake")
            file(READ "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-debug.cmake" DEBUG_CONFIG)
            string(REPLACE "\${_IMPORT_PREFIX}/debug/lib/natsd.dll" "\${_IMPORT_PREFIX}/debug/bin/natsd.dll" DEBUG_CONFIG ${DEBUG_CONFIG})
            file(WRITE "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-debug.cmake" "${DEBUG_CONFIG}")
        endif()
        if(EXISTS "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-release.cmake")
            file(READ "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-release.cmake" RELEASE_CONFIG)
            string(REPLACE "\${_IMPORT_PREFIX}/lib/nats.dll" "\${_IMPORT_PREFIX}/bin/nats.dll" RELEASE_CONFIG ${RELEASE_CONFIG})
            file(WRITE "${CURRENT_PACKAGES_DIR}/share/cnats/cnats-config-release.cmake" "${RELEASE_CONFIG}")
        endif()
    endif()
endif()
