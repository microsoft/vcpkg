...
with open(sys.argv[1], "r") as f_in:
    with open("static_link.bat", "w") as f_out:
        ...
        p_linker = re.compile(rf".+link\.exe.+tensorflow{lib_suffix}\.dll-2\.params.*")
        env = []
        for line in f_in:
                    ...
                    if m:
                        ...
                        for t in tokens:
                            if t.endswith("link.exe"):
                                ...
                                t == "/DLL"
                                or t.lower()[1:].startswith("defaultlib:")
                                or t.lower()[1:].startswith("ignore")
                                or t.startswith("/OPT:")
                                or t.startswith("/DEF:")
                                or t.startswith("/DEBUG:")
                                or t.startswith("/INCREMENTAL:")
                            ):
                                continue
                            elif t[0] == "@" and t.endswith(
                                f"tensorflow{lib_suffix}.dll-2.params"
                            ):
