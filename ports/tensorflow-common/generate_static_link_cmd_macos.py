...
with open(sys.argv[1], "r") as f_in:
    with open("static_link.sh", "w") as f_out:
        p_cd = re.compile("^\\((cd .*) && \\\\$")
        p_linker = re.compile(fr"^\s*(.+)libtool.+(@bazel-out\S+libtensorflow{lib_suffix}\.\d\.\d\.\d\.dylib-2\.params).*")
        ...
        for line in f_in:
            if line.startswith("(cd"):
                ...
                if m1:
                    ...
                    line = f'"{m1.group(1)}libtool" -static -o {m1.group(2)[1:-9].replace(".dylib", ".a")} {m1.group(2).replace(".dylib", ".a")}\n'
                    f_out.write(line)
                else:
                    env.append(line)
