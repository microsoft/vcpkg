...
foreach(BUILD_TYPE IN LISTS PORT_BUILD_CONFIGS)
	...
	if(BUILD_TYPE STREQUAL dbg)
		...
		foreach(OPT IN LISTS VCPKG_C_FLAGS VCPKG_C_FLAGS_DEBUG)
			list(APPEND COPTS "--copt=${OPT}" "--host_copt=${OPT}")
		endforeach()
		...
		foreach(OPT IN LISTS VCPKG_CXX_FLAGS VCPKG_CXX_FLAGS_DEBUG)
			list(APPEND CXXOPTS "--cxxopt=${OPT}" "--host_cxxopt=${OPT}")
		endforeach()
		...
		foreach(OPT IN LISTS VCPKG_LINKER_FLAGS VCPKG_LINKER_FLAGS_DEBUG)
			list(APPEND LINKOPTS "--linkopt=${OPT}" "--host_linkopt=${OPT}")
		endforeach()
	else()
	...
	if(VCPKG_TARGET_IS_WINDOWS)
		...
		if(VCPKG_CRT_LINKAGE STREQUAL "static")
			if(BUILD_TYPE STREQUAL "dbg")
				list(APPEND COPTS --copt=-MTd --host_copt=-MTd)
				list(APPEND CXXOPTS --cxxopt=-MTd --host_cxxopt=-MTd)
				list(APPEND LINKOPTS
					--linkopt=/NODEFAULTLIB:libcmt.lib --host_linkopt=/NODEFAULTLIB:libcmt.lib
					--linkopt=/NODEFAULTLIB:libcpmt.lib --host_linkopt=/NODEFAULTLIB:libcpmt.lib
					--linkopt=/NODEFAULTLIB:libucrt.lib --host_linkopt=/NODEFAULTLIB:libucrt.lib
					--linkopt=/NODEFAULTLIB:libvcruntime.lib --host_linkopt=/NODEFAULTLIB:libvcruntime.lib
				)
			else()
				list(APPEND COPTS --copt=-MT --host_copt=-MT)
				list(APPEND CXXOPTS --cxxopt=-MT --host_cxxopt=-MT)
			endif()
		endif()
