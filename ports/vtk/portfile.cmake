...
if ("all" IN_LIST FEATURES)
    ...
    #Temporary fix for ffmpeg on Apple platforms, which is required to avoid build errors due to missing frameworks.
    #The correct fix would be to add the frameworks to the ffmpeg targets in vcpkg, but this is a quick workaround that doesn't require patching ffmpeg.

    # Fix 1: FFmpeg Apple frameworks — use [[]] to avoid ${ffmpeg_libraries} expansion,
    # and add all frameworks listed in libavcodec.pc
    vcpkg_replace_string("${SOURCE_PATH}/IO/FFMPEG/CMakeLists.txt"
  ...
  PRIVATE
    ...
    "$<$<PLATFORM_ID:Darwin>:-framework VideoToolbox>"
    ...
    # Fix 2: OpenVDB static build doesn't propagate blosc (FindOpenVDB.cmake
    # only adds blosc to HIDDEN_DEPENDENCIES when NOT OPENVDB_USE_STATIC_LIBS)
    vcpkg_replace_string("${SOURCE_PATH}/IO/OpenVDB/CMakeLists.txt"
  ...
  PRIVATE
    ...
    # Fix 3: VTK's FindOpenSlide.cmake creates the target with NO transitive deps.
    # Use pkg-config (openslide.pc lists all required libs) to inject them.
    vcpkg_replace_string("${SOURCE_PATH}/CMake/FindOpenSlide.cmake"
...
set_target_properties(OpenSlide::OpenSlide PROPERTIES
  ...
  if(PKG_CONFIG_FOUND)
    # Determine if openslide was linked as static or shared so we know
    # whether to request --static (Libs.private) from pkg-config.
    get_target_property(_openslide_type OpenSlide::OpenSlide TYPE)
    if(_openslide_type STREQUAL "STATIC_LIBRARY")
      set(_openslide_pkgcfg_static STATIC)
    else()
      set(_openslide_pkgcfg_static "")
    endif()

    pkg_check_modules(_vtk_openslide_deps
      QUIET
      IMPORTED_TARGET
      GLOBAL
      ${_openslide_pkgcfg_static}
      # Core GLib stack
      glib-2.0
      gio-2.0
      gobject-2.0
      ...
      libjpeg
      libpng
      # Compression (pulled in transitively by libtiff)
      zlib
      libzstd
      liblzma
    )

    if(TARGET PkgConfig::_vtk_openslide_deps)
      set_property(TARGET OpenSlide::OpenSlide APPEND PROPERTY
        INTERFACE_LINK_LIBRARIES PkgConfig::_vtk_openslide_deps)
    else()
      message(WARNING
        "OpenSlide: pkg-config found but could not resolve transitive "
        "dependencies. Linking may fail.\n"
        "Hint: set PKG_CONFIG_PATH to the directory containing "
        "glib-2.0.pc, cairo.pc, etc.")
    endif()

    # Clean up helper variables so they don't pollute the cache.
    unset(_openslide_type)
    unset(_openslide_pkgcfg_static)
  else()
    message(WARNING
      "OpenSlide: pkg-config not found. Transitive dependencies (GLib, "
      "cairo, libdicom, …) will not be linked automatically.\n"
      "Hint: install pkg-config or set PKG_CONFIG_EXECUTABLE.")
  endif()
endif ()]]
    )
