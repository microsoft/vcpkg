...
[[    set_target_properties(Qt5::WinMain PROPERTIES
    ...
    if(bins)
        file(COPY ${bins} DESTINATION ${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
    endif()
    vcpkg_copy_tool_dependencies(${CURRENT_PACKAGES_DIR}/tools/qt5/bin)
    if(EXISTS "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/plugins")
    ...
    if(EXISTS "${CURRENT_PACKAGES_DIR}/plugins")
        file(COPY "${CURRENT_PACKAGES_DIR}/plugins" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/qt5")
    endif()
    file(GLOB plugin_dirs "${CURRENT_PACKAGES_DIR}/tools/qt5/plugins/*")
    foreach(plugin_dir IN LISTS plugin_dirs)
        file(GLOB my_plugins RELATIVE "${plugin_dir}" "${plugin_dir}/*")
        vcpkg_copy_tool_dependencies("${plugin_dir}")
        file(REMOVE_RECURSE "${plugin_dir}/plugins")
        file(GLOB deps RELATIVE "${plugin_dir}" "${plugin_dir}/*")
        foreach(dep IN LISTS deps)
            if(NOT "${dep}" IN_LIST my_plugins AND NOT EXISTS "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/${dep}")
                file(RENAME "${plugin_dir}/${dep}" "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/${dep}")
            endif()
        endforeach()
    endforeach()

    file(GLOB ssl_bins "${CURRENT_INSTALLED_DIR}/bin/libcrypto-1_1*.dll" "${CURRENT_INSTALLED_DIR}/bin/libssl-1_1*.dll")
    if(ssl_bins)
        file(COPY ${ssl_bins} DESTINATION "${CURRENT_PACKAGES_DIR}/tools/qt5/bin")
    endif()
    ...
    if(EXISTS ${CURRENT_PACKAGES_DIR}/debug/tools/qt5/bin)
        file(GLOB bins "${CURRENT_PACKAGES_DIR}/debug/bin/*")
        if(bins)
            file(COPY ${bins} DESTINATION ${CURRENT_PACKAGES_DIR}/debug/tools/qt5/bin)
        endif()
