...
function(qt_build_submodule SOURCE_PATH)
    ...
    foreach(_buildname ${BUILDTYPES})
        ...
        if(VCPKG_TARGET_IS_WINDOWS)
            ...
            file(GLOB programs RELATIVE "${CURRENT_BUILD_PACKAGE_DIR}/bin" "${CURRENT_BUILD_PACKAGE_DIR}/bin/*.exe")
            foreach(prog IN LISTS programs)
                file(RENAME "${CURRENT_BUILD_PACKAGE_DIR}/bin/${prog}" "${CURRENT_BUILD_PACKAGE_DIR}/tools/qt5/bin/${prog}")
            endforeach()
            file(GLOB binaries "${CURRENT_BUILD_PACKAGE_DIR}/bin/*")
            if(binaries)
                file(COPY ${binaries} DESTINATION "${CURRENT_BUILD_PACKAGE_DIR}/tools/qt5/bin")
            endif()
        else()
            if(EXISTS "${CURRENT_BUILD_PACKAGE_DIR}/bin")
                file(RENAME "${CURRENT_BUILD_PACKAGE_DIR}/bin" "${CURRENT_BUILD_PACKAGE_DIR}/tools/qt5/bin")
            else()
                file(MAKE_DIRECTORY "${CURRENT_BUILD_PACKAGE_DIR}/tools/qt5/bin")
            endif()
        endif()
    ...
    if(IS_DIRECTORY "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/plugins")
        file(COPY "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/plugins" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/qt5")
        file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/plugins")
    endif()
    if(IS_DIRECTORY "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/qml")
        file(COPY "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/qml" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/qt5")
        file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/tools/qt5/bin/qml")
    endif()
