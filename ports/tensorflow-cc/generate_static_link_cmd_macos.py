...
with open(sys.argv[1], "r") as f_in:
    with open("static_link.sh", "w") as f_out:
        p_linker1 = re.compile("^\\s*(.+)gcc.+-shared.+-o (bazel-out\\S+libtensorflow_cc\\.1\\.14\\.0\\.dylib).+(@bazel-out\\S+libtensorflow_cc\\.1\\.14\\.0\\.dylib-2\\.params).*")
        p_linker2 = re.compile("^\\s*(.+)gcc.+-shared.+-o (bazel-out\\S+libtensorflow_framework\\.1\\.14\\.0\\.dylib).+(@bazel-out\\S+libtensorflow_framework\\.1\\.14\\.0\\.dylib-2\\.params).*")
