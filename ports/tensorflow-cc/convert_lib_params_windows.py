...
with open("tensorflow" + lib_suffix + ".dll-2.params", "r") as f_in:
    lib_name = None
    acc_size = 0
    f_out = open("tensorflow" + lib_suffix + ".lib-2.params", "w")
    for line in f_in:
        if line.startswith("/OUT:"):
            ...
            if acc_size + size > 0xFFFFFFFF:
                ...
                f_out.close()
                file_no += 1
                f_out = open("tensorflow" + lib_suffix + ".lib-2.params-part%d" % file_no, "w")
                acc_size = 0
...
if file_no < required_number:  # vcpkg requires the same number of libs for release build
    ...
    with open("tensorflow" + lib_suffix + ".lib-2.params-part%d" % file_no, "w") as f_out:
        f_out.writelines(lines[:-missing])
    for i in range(missing):
        with open("tensorflow" + lib_suffix + ".lib-2.params-part%d" % (file_no + i + 1), "w") as f_out:
