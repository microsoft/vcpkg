...
function(feature_list_append out)
    cmake_parse_arguments(PARSE_ARGV 1 "arg" "" "FEATURE" "VALUE;VALUE_ELSE")
    if(${arg_FEATURE} IN_LIST FEATURES)
        list(APPEND ${out} "${arg_VALUE}")
    elseif(DEFINED arg_VALUE_ELSE)
        list(APPEND ${out} "${arg_VALUE_ELSE}")
    endif()
    set(${out} ${${out}} PARENT_SCOPE)
endfunction()

function(feature_set out)
    cmake_parse_arguments(PARSE_ARGV 1 "arg" "" "FEATURE;VALUE;VALUE_ELSE" "")
    if(${arg_FEATURE} IN_LIST FEATURES)
        set(${out} "${arg_VALUE}")
    elseif(DEFINED arg_VALUE_ELSE)
        set(${out} "${arg_VALUE_ELSE}")
    endif()
    set(${out} ${${out}} PARENT_SCOPE)
endfunction()
...
list(TRANSFORM CONFIGURE_ENABLED PREPEND "--enable-")
list(TRANSFORM CONFIGURE_DISABLED PREPEND "--disable-")
set(CONFIGURE_ALL ${CONFIGURE_DISABLED})
if(CONFIGURE_ENABLED)
    list(APPEND CONFIGURE_ALL ${CONFIGURE_ENABLED})
endif()
string(JOIN " " CONFIGURE_ALL_JOIN ${CONFIGURE_ALL})
set(OPTIONS "${OPTIONS} ${CONFIGURE_ALL_JOIN}")

if(NOT "decoder-all" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --disable-decoders")
    if(DECODERS)
        string(JOIN "," OPTIONS_DECODERS ${DECODERS})
        set(OPTIONS "${OPTIONS} --enable-decoder=${OPTIONS_DECODERS}")
    endif()
endif()

if(NOT "encoder-all" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --disable-encoders")
    if(ENCODERS)
        string(JOIN "," OPTIONS_ENCODERS ${ENCODERS})
