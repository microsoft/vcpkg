if(NOT COMMAND z_vcpkg_@PORT@_get_link_libraries)
    function(z_vcpkg_@PORT@_get_link_libraries out_var type libraries)
        ...
        set(implicit_link_directories ${CMAKE_C_IMPLICIT_LINK_DIRECTORIES} ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
        list(REMOVE_DUPLICATES implicit_link_directories)
        foreach(lib IN LISTS libraries)
            ...
            elseif(EXISTS "${lib}")
                ...
                if(${lib_var} AND NOT ${lib_var}_implicit)
                    mark_as_advanced("${lib_var}")
                    list(APPEND libs "${${lib_var}}") # full path
                else()
                    find_library("${lib_var}_implicit" NAMES "${lib}" NAMES_PER_DIR PATHS ${implicit_link_directories} NO_DEFAULT_PATH)
                    if(${lib_var}_implicit)
                        mark_as_advanced("${lib_var}")
                        list(APPEND libs "${lib}") # plain name!
                    else()
