...
function(z_vcpkg_gn_expand_targets_recurse gn_target desc_var source_path)
    ...
    elseif(target_type MATCHES "^(executable|shared_library)\$")
        list(APPEND vcpkg_gn_targets_components "${gn_target}")
        set(vcpkg_gn_targets_recurse "RUNTIME")
    elseif(target_type MATCHES "^(loadable_module)\$")
        list(APPEND vcpkg_gn_targets_components "${gn_target}")
    elseif(NOT target_type MATCHES "^(group|source_set)\$")
        set(vcpkg_gn_targets_recurse 0)
    endif()

    z_vcpkg_gn_list_from_json(outputs "${current_json}" "outputs")
    if(outputs STREQUAL "")
        list(APPEND Z_VCPKG_GN_NO_EXPORT "${gn_target}")
        set(Z_VCPKG_GN_NO_EXPORT "${Z_VCPKG_GN_NO_EXPORT}" CACHE INTERNAL "")
        unset(Z_VCPKG_GN_NO_EXPORT)
    endif()
