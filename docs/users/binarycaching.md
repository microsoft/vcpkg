...
| `http,<url_template>[,<rw>[,<header>]]` | Adds a custom http-based location. GET, HEAD and PUT request are done to download, check and upload the binaries. You can use the variables 'name', 'version', 'sha' and 'triplet'. An example url would be<br>'https://cache.example.com/{triplet}/{name}/{version}/{sha}'.<br> Via the header field you can set a custom header to pass an authorization token.
