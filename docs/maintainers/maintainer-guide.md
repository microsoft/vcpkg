...
This files are located in the following locations:
...
_NOTE: These commands require you to have commited your changes to the ports before running them. The reason, is that the Git SHA of the port directory is required in these version files. But don't worry, the `x-add-version` command will warn you if you have local changes that haven't been commited._
