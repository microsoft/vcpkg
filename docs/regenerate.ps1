...
Get-ChildItem "$VcpkgRoot\scripts\cmake" -Filter '*.cmake' | ForEach-Object {
    ...
    $contents = $contents | ForEach-Object {
        if (-not $inComment) {
            if ($_ -match "^\w*${startCommentRegex}(\.[a-z]*)?:?") {
                ...
                Write-Error "Invalid start of comment -- the comment start must be at the beginning of the line.
    (on line: `"$_`")"
            } else {
                # do nothing -- we're outside a comment, so cmake code
            }
        } else {
            if ($_ -match "^\w*#?${endCommentRegex}\w*$") {
                $inComment = $False
                $endCommentRegex = ''
            } elseif ($_ -match $endCommentRegex) {
                Write-Error "Invalid end of comment -- the comment end must be on it's own on a line.
    (on line: `"$_`")"
            } else {
